...
def _compute_indices(spec_at_index_grid: jnp.ndarray, layout: dict,
                     wave_grid: jnp.ndarray) -> jnp.ndarray:
    ...
    for name in names:
        ...
        if name == "D4000":
            ...
            ew = jnp.trapezoid(1.0 - feat_F / jnp.maximum(cont_at_feat, 1e-40),
                               feat_w)
            out.append(ew)
...
def draw_pool(sigma: float, tau: float, ssp_data, obs: Observation):
    ...
    return {"phot": arr_phot, "lines": arr_lines, "indices": arr_indices,
            "full": arr_full}
...
def main() -> None:
    ...
    for sc in SCENARIOS:
        ...
        print(f"{SCENARIO_LABELS[sc]:40s} | "
              f"{z128:7.2f} | {z512:7.2f} | {z2k:7.2f} | {n3:10.0f}")
    ...
    for ax, results, title in [
        ...
        ax.axvline(500, color="orange", ls=":", lw=0.8, alpha=0.5,
                   label="Burnham N=500")
        ax.set_xscale("log")
        ax.set_yscale("log")
