...
def main():
    ...
    print(f"model: D={len(model.spec.free_params)} free scalars, n_grid={model._n_grid}")
    ...
    for i in range(N_GAL):
        ...
        r = f.run("map", key=jax.random.fold_in(jax.random.PRNGKey(1), i),
                  n_steps=12000, n_restarts=3)
        dt = time.time() - t0
        ...
        print(f"   {i}    |     {chi2_n:8.3f}  |   {s_hat:6.3f}   |    {t_hat:8.1f}    | {dt:5.1f}s")
    ...
    print(f"               {'PASS' if ok else 'FAIL'} — per-galaxy fits are "
          f"{'sound' if ok else 'NOT sound; the population step cannot rescue this'}")
    print(f"sigma_hat     : median {np.median(s):.3f} vs truth {TRUTH_SIGMA} "
          f"(study predicts COMPRESSION toward the prior for photometry-only)")
    print(f"tau_hat       : median {np.median(t):.1f} vs truth {TRUTH_TAU_MYR} "
