...
L_radio_t = np.asarray(
    radio_sfr_bell2003(wave_r, L_ir=L_dust_erg, q_ir=2.58, alpha_sf=0.8)
)

fig, (ax_sed, ax_ratio) = plt.subplots(
    2, 1, figsize=(6.0, 5.5), sharex=True,
...
ax_sed.loglog(wave_r / 1e8, nu_r * L_radio_t, color="C3", lw=1.4, ls="--",
              label="tengri radio_sfr_bell2003")
ax_sed.set_ylabel(r"$\nu L_\nu$ [erg s$^{-1}$]")
...
ax_ratio.text(0.02, 0.92, f"median = {np.median(ratio):.3f}",
              transform=ax_ratio.transAxes, fontsize=9, va="top",
...
L_xray_t = np.asarray(
    xray_total(
        wave_x,
        sfr=sfr, stellar_mass=mstar, metallicity_z=metallicity_z,
        stellar_age_gyr=age_m_star_gyr,
        l_2500_30deg=Lnu_2500_30deg,
        gamma_hmxb=2.0, gamma_lmxb=1.6, gamma_agn=1.8, E_cut=300.0,
        delta_alpha_ox=0.0,
...
ax_sed.loglog(E_keV, nu_x * L_xray_t, color="C3", lw=1.4, ls="--",
              label="tengri xray_total")
ax_sed.set_ylabel(r"$\nu L_\nu$ [erg s$^{-1}$]")
...
note = (f"0.5–2 keV: {np.median(ratio[soft]):.3f}\n"
        f"2–10 keV: {np.median(ratio[hard]):.3f}")
ax_ratio.text(0.02, 0.92, note, transform=ax_ratio.transAxes, fontsize=9, va="top",
              bbox=dict(boxstyle="round", facecolor="white", edgecolor="none", alpha=0.8))
ax_ratio.grid(True, which="both", alpha=0.25)
