...
def list_lines(filepath: str) -> None:
    ...
    with h5py.File(filepath, "r") as f:
        ...
        print(f"  Metallicities: {len(log_met)} "
              f"[{log_met[0]:.2f} to {log_met[-1]:.2f}]")
        print(f"  Ages: {len(log_age)} "
              f"[10^{log_age[0]:.1f} to 10^{log_age[-1]:.1f} yr]")
        print(f"  log U: {len(log_U)} "
              f"[{log_U[0]:.1f} to {log_U[-1]:.1f}]")
        print()
    ...
    print(f"{'Idx':>4}  {'Wavelength':>12}  {'Name':<30}  "
          f"{'L (Lsun/Q_H)':>14}")
    print("-" * 70)
