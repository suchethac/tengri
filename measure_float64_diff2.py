...
with tempfile.TemporaryDirectory() as tmpdir:
    ...
    for i, params in enumerate(test_params):
        ...
        abs_diff = np.max(np.abs(lum_new - lum_old))
        rel_diff = np.max(np.abs((lum_new - lum_old) / np.maximum(np.abs(lum_old), 1e-20)))
        max_abs_diff = max(max_abs_diff, abs_diff)
        max_rel_diff = max(max_rel_diff, rel_diff)

        print(f"Point {i+1}:")
        print(f"  max|new-old|        = {abs_diff:.3e}")
        print(f"  max|new-old|/|old|  = {rel_diff:.3e}")
        print(f"  Example lum_old[0]  = {lum_old[0]:.6e}")
        print(f"  Example lum_new[0]  = {lum_new[0]:.6e}")

    print()
    print(f"Overall max absolute difference: {max_abs_diff:.3e}")
