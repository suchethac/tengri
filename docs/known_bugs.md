...
| IMP-11 | Sphinx `docs/api/models.rst` should be renamed to `components.rst` with module paths updated to `tengri.components.*`. Auto-generated gallery `.rst` files will regenerate on next Sphinx build. | OPEN — cosmetic |
| IMP-12 | `inference/backends/` subpackages (`vi/`, `mcmc/`) are scaffolded with `__init__.py` stubs but inference backend files (`vi.py`, `geovi.py`, `nuts.py`, `raytrace.py`, `elliptical_slice.py`, `map_optimizer.py`, `map_dispatch.py`, `laplace.py`, `pathfinder.py`, `sbi.py`, `mcmc.py`) have not yet been moved into them. Current flat `inference/` layout still works. | OPEN — cosmetic |
