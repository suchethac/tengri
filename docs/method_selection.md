...
| Nonparametric SFH (`continuity`, `dirichlet`; D ≈ 9) | `mcmc_nuts` with `dense_mass_matrix=True` | The bin-ratio posterior is correlated, and NUTS on a diagonal mass matrix saturates its tree depth there: measured on a 9-D continuity photometry fit, diagonal cost ~4× the wall time and ~2× the seconds per effective sample of dense. The `dense_mass_matrix=None` auto-policy switches to diagonal at D ≥ 8, so pass `True` explicitly. |
| Nonparametric SFH (`continuity`, `dirichlet`; D ≈ 9) | `mcmc_nuts`, mass matrix left at the default | The bin-ratio posterior is correlated, but neither shortcut for correlated geometry survives contact with it. Measured on a 9-D `continuity` fit to 19 JWST bands, 400 samples each: the diagonal default gave min ESS 165 with 7 divergences; `dense_mass_matrix=True` gave min ESS 10 with 77 divergences in a third of the wall time; `mcmc_hmc` at the validated settings above gave min ESS 4 in a ninth of it. Budget a few minutes per galaxy and rank by ESS, not wall time. In catalog mode the batched NUTS program spent over fifteen minutes in XLA compilation on the same model without sampling, because a per-step trajectory length does not vectorize cheaply; use `mcmc_hmc` with `n_leapfrog_steps=150` there and treat the result as screening. |
