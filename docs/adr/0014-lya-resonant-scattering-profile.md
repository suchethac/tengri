...
|---|---|---|
| `'delta'` (default) | None — preserves current behaviour | 0 |
...
`'delta'` is the no-op for existing fits — semantically the current behaviour. `'verhamme2006'` is the table-based pilot; `'gronke2015_emulator'` is the differentiable / extensible follow-up.
...
- **Scalar entry**: ``(wave, luminosity)`` — current behaviour, used for every non-Lyα species.
...
`neb_fesc_lya` continues to scale the *integrated* Lyα luminosity: the profile is computed first (shape from N_HI / v_outflow), then renormalised so its integral equals the scalar `(1 − fesc_lya) × L_lya_intrinsic`. This keeps the existing energy-balance contract; only the *shape* is new.

### Dust attenuation

Profile-bearing entries flow through `attenuate_emission()` exactly like the existing `all_lums` catalogue did in #320 (PR landed in cs/fix-nebular-surface). Each pixel of the profile multiplies by the attenuation at its wavelength. Birth-cloud vs diffuse selection follows the same `_neb_dust_mode` rule as the rest of the nebular surface.
...
- **Differentiable** — the NN emulator path makes N_HI / v_outflow inferrable from data via the standard JAX gradient path. The table path supports forward modelling only (returns NaN gradients for these parameters).
...
- Default behaviour unchanged (`type='delta'`).
...
Phase 1 is the smallest closes-the-issue PR; phase 2 is the differentiable-fitting upgrade; phase 3 is the documentation/visualisation deliverable.
