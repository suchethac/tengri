...
| M1c Fritz 2006 §9e | cigale §9e 0.141× and 0.192× on the two in-range nodes, 13/13 bands outside 5 % | **Not an AGN residual — the panel is mis-scoped.** The ratio is taken over the *full* SED (stellar + host dust + disc + polar screen + torus) through IR filters, at nodes where pcigale's torus carries only ~8 % of the total. So a heading that reads "Fritz torus" is reporting a host-side difference: the torus arm itself agrees to about 1.0×, and the non-torus remainder is ~6.75 % of pcigale's. The SKIRTOR panel beside it, same host and same metric, reads 0.97× because its torus dominates the bands and masks the same host deficit. Fix the comparison first — compare the torus component, as the agnfitter notebook's §9c does — then chase the host IR separately. **Caveat:** the "torus ≈ 1.0×" arm was measured on a reconstructed build, not on the notebook's own `m_fritz_sweep`, and one node's share was not measured; the mis-scoping is established, the host number is provisional. |
| Out-of-prior node | cigale §9e second Fritz node | `fritz_oa` is declared `Uniform(20.0, 80.0)` at `reproduction/cigale/01_cigale.py:3287` while the node evaluates **oa = 100.0** — outside the parameter's own declared prior, so its 0.244× is extrapolation. Same disease as the off-grid sweep nodes already corrected on this branch. Note `tools/check_param_ranges.py` exists for exactly this class and does not catch it: the guard compares a **call-site prior declaration** against the registry declaration, while this is a **runtime evaluation value** passed into a swept prediction. A sweep node outside a declared prior is a shape that guard cannot reach. |
