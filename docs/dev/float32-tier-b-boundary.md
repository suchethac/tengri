...
`custom_vjp`** in `stellar/component.py` that reorder only the reverse pass:
...
1. **Mass scaling** `total_mass · <per-Msun SSP> · L_sun` (~18 phot/spectrum-LUT
   ...
   simplifier pulls the constant back out — a `custom_vjp` boundary is the one
   thing reassociation cannot cross. (The full-wave SED keeps its
   `L_sun`-in-einsum fold, which the custom_vjp cannot replace there: a unit
   cotangent times `total_mass · L_sun` still overflows, so the fold — Jacobian
   `total_mass` ~1e10 — is required for the SED, the custom_vjp for the LUT.)
