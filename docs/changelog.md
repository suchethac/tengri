...
- **Nebular**: MAPPINGS V stellar and AGN photoionization grids (Flury et al. 2024, `neb={'type': 'mappings'}` or `'mappings_agn'` — **both backends are registered as experimental; both refuse loudly pending data rehabilitation** (#2082): stellar grid is 51.2% NaN (2656/5184 cells in logHB_per_logq, low-metallicity bias), AGN backend lacks the model protocol surface; precompute adapters also defective pending #2078), MAPPINGS V shock emission lines (Allen+2008), diffuse ionized gas (DIG) mixing, analytic calibration polynomial marginalization (Johnson+2021)
- **Inference**: Laplace approximation, Pathfinder, Elliptical Slice Sampling, Nested Slice Sampling (NSS)

### Bug Fixes
- **JAX tracer safety in nebular interpolation (2026-08, found while wiring #2070)**: `_interp_index_weight` used direct numpy-style indexing `grid[idx + 1]` which failed when `idx` was a traced value under `jax.vmap`. Fixed by replacing with `jnp.take(grid, idx + 1)`, which is JAX-safe. The bug was latent in all calls inside vmap (e.g., `predict_nebular_line_luminosities` over ages) but only surfaced when the grid data was complete enough to reach that code path.
