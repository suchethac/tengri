...
:alt: A real SSP spectrum under the GALEX and ugriz bandpasses, one band split into five equal-mass sub-bands, each collapsing to one stored number, and the runtime contraction against the dust screen.
:width: 100%

What `approx=WavePrecomp()` does to a real spectrum. **(a)** An SSP template
under the GALEX and ugriz bandpasses; SDSS *u* is followed through the rest
of the figure. **(b)** The band is cut into $K = 5$ sub-bands of *equal
filter mass* — the five shaded regions have equal area under $T_b w$, so
they are narrow where the filter is most transmissive and wide out in the
wings. Each sub-band gets one node $\lambda^\star_k$, the template's own
flux-weighted centroid there (red ticks). **(c)** The area under
$F_\nu T_b w$ within each sub-band collapses to a single stored number
$\Phi_k$ — the orange bars have exactly that area — turning the band's 667
wavelength points into five numbers and five nodes, per (metallicity, age,
band). Nothing here depends on a free parameter, so it is all built once.
**(d)** Every likelihood call then evaluates the dust screen at those five
nodes, scales the stored bars, and sums. No wavelength grid is touched, and
for this band the result sits within a few thousandths of a percent of the
exact integral, against $-0.56\%$ for the single-effective-wavelength scheme
it replaces. Generated by `scripts/render_precompute_schematic.py`.
```
