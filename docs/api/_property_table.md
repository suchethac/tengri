...
| `civ_1549` | erg/s | lines | nebular | CIV 1549 line luminosity. On cue's legacy 128-line subset (full_catalog=false) no catalog line falls within tolerance of this wavelength, so the value is not a number, with a warning (#2239). |
...
| `log_civ_1549` | dex | lines | nebular | log10 of civ 1549 line luminosity. on cue's legacy 128-line subset (full_catalog=false) no catalog line falls within tolerance of this wavelength, so the value is not a number, with a warning (#2239). [dex re erg/s]; float32-safe form of `civ_1549` |
