#!/usr/bin/env python3
"""Probe compile size for AGN models with VI inference.

Tests if large constants only appear during VI/gradient compilation,
not just forward photometry.

Usage:
    JAX_PLATFORMS=cpu .venv/bin/python tools/probe_agn_vi_compile.py
"""
...
def probe_gradient(agn_model: str | None, ssp_data) -> dict[str, Any]:
    """Probe the gradient/JVP compile path (used in VI and MCMC)."""
    print(f"\n=== agn_model = {agn_model!r} (gradient path) ===", flush=True)
    t0 = time.perf_counter()
    try:
        fitter = _make_fitter(agn_model, ssp_data)
        fitter._compilation_event.wait(timeout=0.0)
    ...
    t0 = time.perf_counter()
    try:
        # Compute JVP (derivative) to trigger constant folding in XLA
...
def main() -> int:
    ...
    for agn in cases:
        try:
            rows.append(probe_gradient(agn, ssp_data))
        except Exception as e:
            print(f"  EXCEPTION: {type(e).__name__}: {e}")
            rows.append({"agn_model": agn, "path": "gradient", "error": f"{type(e).__name__}"})
