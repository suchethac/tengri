...
def _model(tau_diff, tau_bc):
    return tengri.SEDModel.build(
        ...
        dust={"type": "two_component", "*": tengri.FIXED,
              "tau_diff": tau_diff, "tau_bc": tau_bc, "slope": -0.7},
        redshift=tengri.Fixed(0.05),
    )


fig, (ax_bc, ax_diff) = plt.subplots(1, 2, figsize=(11.5, 4.4),
...
for tau in tau_bc_vals:
    ...
    ax_bc.loglog(wave, C_AA_PER_S / wave * np.asarray(out.sed),
                 color=plt.cm.viridis(norm_bc(tau)), lw=1.4)
...
for tau in tau_diff_vals:
    ...
    out = model.predict_rest_sed(p)
    wave = np.asarray(out.wavelength)
    ax_diff.loglog(wave, C_AA_PER_S / wave * np.asarray(out.sed),
                   color=plt.cm.viridis(norm_diff(tau)), lw=1.4)

for ax in (ax_bc, ax_diff):
    ax.set(xlim=(800, 3e4), ylim=(1e41, 5e44),
           xlabel=r"Rest-frame wavelength $\lambda$ [$\mathrm{\AA}$]")

ax_bc.set_ylabel(r"$\nu L_\nu$  [erg s$^{-1}$]")
ax_bc.text(0.05, 0.93, r"sweep $\tau_{\rm bc}$, fixed $\tau_{\rm diff}=0.2$",
           transform=ax_bc.transAxes, fontsize=9, color="0.15",
           bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="0.7", lw=0.5))
ax_diff.text(0.05, 0.93, r"sweep $\tau_{\rm diff}$, fixed $\tau_{\rm bc}=0.5$",
             transform=ax_diff.transAxes, fontsize=9, color="0.15",
             bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="0.7", lw=0.5))

cb_bc = fig.colorbar(plt.cm.ScalarMappable(norm=norm_bc, cmap="viridis"),
                     ax=ax_bc, pad=0.01)
cb_bc.set_label(r"$\tau_{\rm bc}$  [mag]")
cb_diff = fig.colorbar(plt.cm.ScalarMappable(norm=norm_diff, cmap="viridis"),
                       ax=ax_diff, pad=0.01)
