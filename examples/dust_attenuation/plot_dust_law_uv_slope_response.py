...
ax.axhline(beta_intrinsic, color="0.55", lw=0.7, ls="--",
...
for law_key, label, color in LAWS:
    ...
    ax.bar(label, beta - beta_intrinsic, color=color,
           edgecolor="0.15", lw=0.5)
    ax.text(label, beta - beta_intrinsic + 0.05,
            f"{beta:+.2f}", ha="center", fontsize=8, color="0.2")

ax.set(ylabel=r"$\Delta\beta$ vs intrinsic  (Calzetti+1994 fit)",
       ylim=(0, ax.get_ylim()[1] * 1.15))
ax.set_xticklabels([lbl for _, lbl, _ in LAWS], rotation=20, ha="right",
                   fontsize=8.5)
ax.legend(frameon=False, fontsize=9, loc="upper right")
ax.text(0.02, 0.97, r"applied $\tau_V = 1$", transform=ax.transAxes,
