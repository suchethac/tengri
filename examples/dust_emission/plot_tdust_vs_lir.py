...
for u in umin_vals:
    L_nu = np.asarray(
        draine_li2007(wave_aa, u * L_ABS_0,
                      dust_umin=u, dust_gamma_dl=0.01, dust_qpah=2.5)
    )
    seds.append(L_nu)
    # νL_ν peak → Wien T
...
ax_corr.scatter(L_IR, T_peak, c=np.log10(umin_vals),
                cmap=cmap, s=80, edgecolor="k", lw=0.4, zorder=3)
ax_corr.set(xscale="log", xlabel=r"$L_{\rm IR}$ (8–1000 $\mu$m) [erg s$^{-1}$]",
...
ax_sed.set(xlim=(3, 500), xlabel=r"$\lambda$ [$\mu$m]",
           ylabel=r"$\nu L_\nu$ [erg s$^{-1}$]")
