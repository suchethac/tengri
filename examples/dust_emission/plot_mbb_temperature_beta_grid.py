...
_, p_check = _build()
mbb_keys = sorted(k for k in p_check if k.startswith("dust_") and
                  ("temp" in k.lower() or "beta" in k.lower() or "t_dust" in k.lower()))
...
fig, (ax_T, ax_b) = plt.subplots(2, 1, figsize=(7.2, 6.4), sharex=True,
...
for T in T_grid:
    ...
    ax_T.loglog(w, C_AA_PER_S / w * np.asarray(out.sed),
                color=plt.cm.plasma(norm_T(T)), lw=1.4)
...
for beta in beta_grid:
    ...
    out = model.predict_rest_sed(p)
    w = np.asarray(out.wavelength)
    ax_b.loglog(w, C_AA_PER_S / w * np.asarray(out.sed),
                color=plt.cm.viridis(norm_b(beta)), lw=1.4)

for ax in (ax_T, ax_b):
    ax.set(xlim=(1e5, 1e7), ylim=(1e40, 1e44),
           ylabel=r"$\nu L_\nu$  [erg s$^{-1}$]")

ax_b.set_xlabel(r"Rest-frame wavelength $\lambda$ [$\mathrm{\AA}$]")

cb_T = fig.colorbar(plt.cm.ScalarMappable(norm=norm_T, cmap="plasma"),
                    ax=ax_T, pad=0.01)
cb_T.set_label(r"$T_{\rm dust}$  [K]   (β = 1.8 fixed)")
cb_b = fig.colorbar(plt.cm.ScalarMappable(norm=norm_b, cmap="viridis"),
                    ax=ax_b, pad=0.01)
