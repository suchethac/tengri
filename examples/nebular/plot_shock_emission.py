...
v_shock_range = np.linspace(100, 500, 15)
...
for v_shock, color in zip(v_shock_range, colors):
    try:
        line_ratios = shock_line_ratios(
            shock_velocity_kms=v_shock,
            shock_density_cm3=100.0,
            magnetic_param_b=1.0,
        )
        # Extract [O III]/Hβ and [N II]/Hα
        log_oiii_hb = np.log10(
            line_ratios.get("OIII5007", 1.0) / line_ratios.get("Hbeta", 1.0)
        )
        log_nii_ha = np.log10(
            line_ratios.get("NII6584", 1.0) / line_ratios.get("Halpha", 1.0)
        )
...
for dens, color in zip(density_range, colors):
    try:
        line_ratios = shock_line_ratios(
            shock_velocity_kms=200.0,
            shock_density_cm3=dens,
            magnetic_param_b=1.0,
        )
        log_oiii_hb = np.log10(
            line_ratios.get("OIII5007", 1.0) / line_ratios.get("Hbeta", 1.0)
...
v_shock_dense = np.linspace(100, 500, 20)
line_flux_dict = {
    "[O III]5007/Hβ": [],
    "[O I]6300/Hα": [],
    "[S II]6724/Hα": [],
...
for v_shock in v_shock_dense:
    try:
        ...
        line_flux_dict["[O I]6300/Hα"].append(
            line_ratios.get("OI6300", 1.0) / line_ratios.get("Halpha", 1.0)
        )
        line_flux_dict["[S II]6724/Hα"].append(
            line_ratios.get("SII6724", 1.0) / line_ratios.get("Halpha", 1.0)
        )
    except Exception:
        pass
