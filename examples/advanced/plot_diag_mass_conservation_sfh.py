...
# DPL parameter grid: vary (alpha, beta, tau_gyr, log_peak_sfr)
alpha_vals = [1.0, 1.5, 2.5]
beta_vals = [0.8, 1.5]
tau_gyr_vals = [0.5, 2.0]
log_peak_sfr_vals = [0.0, 1.0, 2.0]
        ...
        for tau_gyr in tau_gyr_vals:
            for log_peak_sfr in log_peak_sfr_vals:
                ...
                params = {
                    ...
                    "sfh_dpl_log_peak_sfr": log_peak_sfr,
                    "redshift": 0.0,
                }
                ...
                log_peak_sfr_colors.append(log_peak_sfr)

m_manual = np.array(m_manual_list)
m_lib = np.array(m_lib_list)
log_peak_sfr_colors = np.array(log_peak_sfr_colors)
...
norm = plt.Normalize(vmin=log_peak_sfr_colors.min(), vmax=log_peak_sfr_colors.max())
colors = cmap(norm(log_peak_sfr_colors))
