...
class Observation:
    ...
    def _apply_spec_calibration(self, flux, wave_obs, params):
        ...
        Applied after the LSF convolution: the polynomial models the response of
        the instrument that the LSF has already smeared, not a property of the
        intrinsic spectrum.
        """
    ...
    def predict_spectrum_via_precomp(
        ...
        **JIT-compatible**: yes — pure JAX arithmetic on the LUT entries.
        ...
        # spectrum only: it models the instrument's calibration error, which has
        # no meaning for the rest-frame (z=0, 10 pc) spectrum. Omitting it here
        # would silently drop calibration under SpectrumPrecomp while the exact
        # path applied it — the LUT-drops-a-component class of #737/#740.
