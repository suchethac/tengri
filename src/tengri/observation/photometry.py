...
def compute_flux_density(
    ...
    **JIT-compatible**: yes — all operations are ``jnp`` primitives;
    ``convention`` is a static argument. Safe to call inside :func:`jax.jit`.
    ...
    **Filter convolution formula** (photon-counting, ``BESSELL`` default):
    ...
    (``getmags.f90``; Fukugita+1996 Eq. 7) and DSPS (Hearin+2023, with
    :math:`T` = photon transmission probability; Hogg+2002 Eq. 5). The
    ``ENERGY`` convention replaces :math:`1/\\lambda` with :math:`1/\\lambda^2`
    ...
    **Edge handling**: :math:`L_\\nu = 0` outside the SED wavelength domain
    ...
    # Bandpass weight w(lambda): 1/lambda (Bessell) or 1/lambda^2 (energy).
    weight = filter_trans * _filter_weight(filter_wave, convention)

    # Filter-weighted integral: int(SED * T * w dlam) / int(T * w dlam)
    numerator = jnp.trapezoid(sed_on_filter * weight, filter_wave)
    denominator = jnp.trapezoid(weight, filter_wave)
...
def pad_filters_to_bucket(filter_waves: list, filter_trans: list):
    ...
    (the integrand is ``trans × w(λ) × SED`` and the denominator divides
...
def _compute_flux_density_padded(
    ...
    :func:`_filter_weight` maps ``wave=0`` to weight 0), so no masking is
