...
-----------------------------
Implements the ``composable`` top-level factory and all five
sub-block submodules. The other 13 top-level AGN models (``simple``,
``standard``, ``multicolor_agn``, ``unified_nlr_blr``, ``skirtor``,
``grahsp``, ``qsogen``, ``relagn``, ``adaf``, ``cat3d_wind``,
``silva04``, ``kubota_done``, ``kubota_done_full``) are reachable via
the dict form (``{'type': 'simple', '*': FREE, ...}``) until factory
support follows in a subsequent PR.
"""
...
def available() -> list[str]:
    ...
    Currently lists ``"composable"`` (the orchestrator that consumes
    the five sub-block kwargs). Other top-level AGN models can still
    be built via the dict form; first-class factories for them are a
    follow-up.
    """
    return ["composable"]
