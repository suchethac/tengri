...
class SEDModel:
    ...
    **IGM absorption gotcha**: :meth:`predict_obs_sed` applies IGM transmission
    ...
    def compile_signature(self) -> tuple:
        ...
        # Per-source dust-screen choice (#2234 replacement): which of
        # the two screens (or neither) attenuates the nebular continuum/line
        # catalog and the shock SED. Changes the baked tau assembly inside
        # DustSEDComponent.apply (birth_cloud vs diffuse vs identity) but not
        # the graph shape, so two models differing only here MUST get
        # distinct signatures or the kernel cache leaks one model's screen
        # choice into the other (color-leak), exactly like
        # ``dust_law_overrides_sig`` below.
        dust_nebular_screen_sig = str(getattr(self, "_dust_nebular_screen", "birth_cloud"))
        dust_shock_screen_sig = str(getattr(self, "_dust_shock_screen", "diffuse"))
        dust_agn_screen_sig = str(getattr(self, "_dust_agn_screen", "none"))
        ...
        return (
            ...
            dust_law_diff_fn_name,
            dust_law_neb_name,
            dust_nebular_screen_sig,
            dust_shock_screen_sig,
            dust_agn_screen_sig,
            wg00_selectors,
