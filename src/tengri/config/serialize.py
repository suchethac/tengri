...
def deserialize_config(
    config: ConfigDict, *, strict: bool = True
) -> ConfigDict:
    ...
    strict : bool, optional
        If True (default), raise on unknown distribution names or malformed
        dicts. If False, log warnings and pass through.
    ...
    """
    return _deserialize_recursive(config, path=[], strict=strict)
...
def _deserialize_recursive(
    ...
    if isinstance(obj, str):
        ...
        if "__fixed__" in obj:
            ...
            return dict_to_distribution(obj, path=path, strict=strict)
        else:
            # Regular dict, recurse
            return {
                k: _deserialize_recursive(v, path + [k], strict=strict)
                for k, v in obj.items()
            }
    elif isinstance(obj, (list, tuple)):
        return [_deserialize_recursive(item, path, strict=strict) for item in obj]
    else:
        # Scalars pass through unchanged
        return obj
...
def dict_to_distribution(
    ...
    if prior_name not in dist_map:
        ...
        _raise_config_error(path, msg, strict=strict)
        if not strict:
...
def load_config_from_file(
    ...
    return deserialize_config(raw, strict=strict)
