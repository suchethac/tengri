...
def compare_paths(
    ...
    filters,
    n: int = 200,
) -> tuple[PipelineReport, PipelineReport]:
    """Compare exact vs fused pipeline performance for the same model.
    ...
    Returns
    -------
    exact_report, fused_report : tuple[PipelineReport, PipelineReport]
    """
    import warnings
    ...
    print()
    print(fused.summary())
    speedup = exact.total_us / fused.total_us if fused.total_us > 0 else float("inf")
    print(f"\nSpeedup (fused/exact): {speedup:.1f}x")

    if exact.gradient_us and fused.gradient_us:
        grad_speedup = exact.gradient_us / fused.gradient_us
...
def profile_configurations(
    ...
    """Profile multiple model configurations.
