...
class ConstrainedMCMCInfo(NamedTuple):
    ...
    ----------
    info
        The underlying MCMC info.
    is_accepted
        True if the MCMC proposal was accepted AND the proposed point is above
...
def update_with_mcmc_take_last(
    ...
    """Update strategy: use MCMC to replace particles, keep final state only.
    ...
    ----------
    constrained_mcmc_step_fn
        Wrapped MCMC step function enforcing the NS likelihood constraint.
    num_mcmc_steps
...
def build_kernel(
    ...
    """Build a NS kernel wrapping any MCMC algorithm.
    ...
    logdensity_fn
        Log-density function (typically the prior).
    mcmc_init_fn
        Function to initialize MCMC state from position and logdensity_fn.
    mcmc_step_fn
        MCMC step: (rng_key, state, logdensity_fn, **params).
    num_inner_steps
    ...
    def constrained_mcmc_step_fn(rng_key, state, loglikelihood_0, **params):
        """Single constrained MCMC step respecting the likelihood threshold."""
        mcmc_state = mcmc_init_fn(state.position, logdensity_fn)
        new_mcmc_state, mcmc_info = mcmc_step_fn(rng_key, mcmc_state, logdensity_fn, **params)
