...
def _data_fingerprint(fitter: Any) -> str:
    ...
    Derived from every Fitter attribute under
    ``tengri.inference._engine_policy.FINGERPRINT_POLICY``: exactly the
    five data rows (``data``, ``noise``, ``data_mask``, ``presence``,
    ``_runtime_redshift``) are hashed by content; every other Fitter
    attribute is excluded there as structure, already covered by
    ``tengri.inference._engine_policy.ENGINE_POLICY`` (see that module's
    docstring for the full complementarity rule).
    ...
    -------
    str
        Hex digest, process-stable (BLAKE2b, not Python's per-process-salted
        ``hash()``). Cost is linear in the data bytes, negligible beside the
        thousands of forward passes a MAP run would otherwise repeat.
    """
    from tengri._cache_keys import derive_key, stable_digest
    from tengri.inference._engine_policy import FINGERPRINT_POLICY, FINGERPRINT_VERSION

    key = derive_key(fitter, FINGERPRINT_POLICY, version=FINGERPRINT_VERSION)
    return stable_digest(repr(key).encode())
