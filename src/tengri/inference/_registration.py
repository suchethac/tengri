...
register_backend(
    ...
    short_doc=(
        ...
        "rank is however many L-BFGS steps actually ran. TWO mechanisms predict "
        "the collapse and the report did not separate them -- a low-rank "
        "correction too small for a posterior at cond 1e5-1e8, or a correction "
        "that is EMPTY because tengri seeds from the MAP and L-BFGS started at "
        "an optimum has no path to record. The second is the one upstream "
        "documents: blackjax's own Pathfinder page advises a deliberately BAD "
        "initialization 'in order to make the L-BFGS algorithm run longer and "
        "have more datapoints to estimate the inverse hessian matrix', and "
        "starting at an exact mode empties beta entirely (0 of 20 columns, "
        "measured). precondition=True roughly halves the collapse (worst 0.11 -> 0.21) "
