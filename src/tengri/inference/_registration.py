...
register_backend(
    ...
    short_doc=(
        ...
        "rank is however many L-BFGS steps actually ran. The EXPLANATION is not "
        "settled: a low-rank correction too small for a posterior at cond "
        "1e5-1e8 is the surviving candidate, not a demonstrated cause. The rival "
        "-- that the correction is empty because tengri seeds from the MAP, "
        "which blackjax's own Pathfinder page warns against -- was tested and "
        "REFUTED: dropping the MAP seed made the widths worse on 8 of 8 "
        "parameters (median ratio 0.48 -> 0.16), so MAP seeding is the better "
        "default and stays. To confirm the survivor, read beta's effective rank "
        "out of a real fit; nothing does that yet. "
