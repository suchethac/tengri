...
def _refuses_feature_precomp(model) -> bool:
    ...
    THE one place the top-up's refusals are stated. Two surfaces attempt it on
    a model that already carries a build-time ``approx=`` —
    :meth:`Fitter._add_feature_precomp` and the ``wave_precomp`` branch of
    :func:`_resolve_batch_fit_approx` — and a refusal only one of them knows
    about is a surface that silently disagrees with the other. That is the
    shape of #1683 (an early return one surface had and the other did not) and
    of #1691 (a channel two readers resolved differently); restating the
    condition per call site is how both were written.

    Refused on two grounds:

    * the LUT is already configured, so there is nothing to add;
    * the observation carries a line-adjacent channel (#1665) — the ratio and
      ...
      outranks the speedup, so this ground is load-bearing.

    ``feature_precomp`` is read with :func:`getattr` rather than by attribute:
    these state objects are not uniformly populated, matching how
    ``spectrum_precomp`` is read in the batch resolver.
    ...
    -------
    bool
        ``True`` when the top-up must be skipped. A ``False`` says only that
        the policy permits it — the attempt can still fail, and that failure
        is the caller's to handle.

    Notes
    -----
    Not a JAX function; called once per fit at resolution time.

    A model with no approx state at all is refused here, because there is no
    build-time config to top up. The batch resolver's other branch — the one
    that has yet to add the wave LUT — therefore does NOT consult this, and
    checks :func:`_has_line_adjacent_channel` directly.
    """
    state = getattr(model, "approx", None)
    if state is None or getattr(state, "feature_precomp", False):
        return True
    ...
    if isinstance(approx, str):
        ...
        if data_type == "photometry":
            if state is not None and state.wave_precomp:
                ...
                # LUT would duplicate it. Same refusals as the single-galaxy
                # surface, read from the same predicate rather than restated.
                if _refuses_feature_precomp(model):
                    return model
...
class Fitter:
    ...
    **Engine caching**: Compiled engines are cached on the Model object so that
    ...
    def _add_feature_precomp(self, model, *, warn_on_failure=True):
        ...
        -------
        SEDModel
            The topped-up clone, or ``model`` unchanged when
            :func:`_refuses_feature_precomp` says the top-up must not happen,
            or when it could not be enabled.

        Notes
        -----
        The refusals are not stated here: they live in
        :func:`_refuses_feature_precomp`, shared with the batch resolver, so
        the two surfaces cannot drift apart on which models are eligible.
    ...
    def _resolve_fit_approx(self, model: Any, approx):
        ...
        if isinstance(approx, str):  # "auto"
            ...
            if callable(has) and has():
                ...
                # below, past the early return — was unreachable for any model
                # built with approx=WavePrecomp(): naming the wave LUT
                # explicitly cost the feature LUT, 22x in likelihood-gradient
                # FLOPs on a 10-parameter Cue model. Failure is expected for a
                # backend with nothing to tabulate, so only the lines case —
                # whose cost is the documented ~21x — announces it.
                #
                # The line-adjacent exclusion is NOT repeated here: it lives
                # inside ``_add_feature_precomp``, one seam for every caller
                # of the top-up (#1665).
                resolved = model
