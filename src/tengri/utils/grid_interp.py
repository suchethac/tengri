...
def preintegrate_grid(
    ...
    weight ``w = 1/λ``; ``ENERGY`` uses ``w = 1/λ²``):
        Φ = ∫ L_ν(λ_obs) T_b(λ_obs) w(λ_obs) dλ / ∫ T_b(λ_obs) w(λ_obs) dλ

    This matches :func:`tengri.observation.photometry.compute_flux_density`
    so the WavePrecomp path and the exact path agree per convention.
    ...
    Ψ ≡ 0 for a flat template (the Taylor expansion is centred consistently).
