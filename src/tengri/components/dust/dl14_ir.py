...
"""Draine & Li (2014) dust emission as an SEDModelComponent.

Implements tabulated-template IR re-emission based on the DL14 dust model update,
...
.. [2] Draine, B.T. et al. 2014 update (CIGALE implementation, Boquien+2019).
"""
...
__all__ = ["DL14IRConfig", "DL14IRSEDComponent"]


@dataclass(frozen=True)
class DL14IRConfig(SEDComponentConfig):
    """Configuration for DL14IR dust emission component.

    Attributes
    ----------
    name : str
        Diagnostic identifier. Default ``"dl14_ir"``.
    template_path : str, optional
        Path to DL14 template HDF5 file. If None, attempts to auto-locate
...
class DL14IRSEDComponent(SEDModelComponent):
    """Dust IR emission via tabulated Draine & Li (2014) templates.
    ...
    precomputed template grid (updated DL14 parameterization).

    Attributes
    ----------
    name : str
        Stable identifier: ``"dl14_ir"``.
    ...
    **Pipeline ordering**: This component MUST run after dust attenuation.
    Typical order: ``[Stellar, Nebular, DustAttenuation, DL14, IGM, Radio]``.
    """

    name = "dl14_ir"
    parameter_prefix = "dust_"
    config: DL14IRConfig = DL14IRConfig()
    ...
    def load(self, wave: jnp.ndarray | None = None) -> Any | None:
        """Preload DL14 template grid.

        Called at model init time (not JIT-ed). Attempts to locate and load
        the DL14 template HDF5 file. If unavailable, returns None and the
        component will skip gracefully.

        Parameters
        ----------
        wave : ndarray, optional
            Rest-frame wavelength grid in Angstrom (not used for DL14
        ...
        if template_path is None:
            warnings.warn(
                f"DL14IR component {self.name!r}: template file not found. "
        ...
        try:
            ...
            warnings.warn(
                f"DL14IR component {self.name!r}: failed to load templates: {e}. "
                "Component will skip.",
                UserWarning,
                stacklevel=2,
            )
    ...
    def predict(
        ...
        r"""Compute dust emission via DL14 tabulated templates.

        Interpolates the DL14 template grid over (q_PAH, U_min, gamma) and
        ...
        sed_in : ndarray, shape (n_wave,)
            Input SED in erg/s/Hz (ignored; DL14 emission is computed
