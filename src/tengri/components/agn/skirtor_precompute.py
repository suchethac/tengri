...
def _register_skirtor_component():
    """Register SKIRTOR AGN component spec at module load time."""
    ...
    def _precompute_skirtor(filter_waves, filter_trans, redshift, parameters):
        """Wrapper to find grid and precompute SKIRTOR photometry."""
        try:
    ...
    def _extract_skirtor_arrays(precomp):
        """Extract JIT-traceable arrays from SKIRTOR precompute result."""
        if precomp is None or "_preint" not in precomp:
            return ()
        return (precomp["grid_phot"], precomp["axes"])

    register(
        ComponentSpec(
            name="agn:skirtor",
            precompute=_precompute_skirtor,
            extract_arrays=_extract_skirtor_arrays,
            build_lookup=lambda precomp, *, grid_arrays_traced=None: build_lookup(precomp),
            apply_signature=(
                "agn_tau_skirtor",
                "agn_p_skirtor",
                "agn_q_skirtor",
            ...
            activation=lambda spec, parameters: (
                hasattr(parameters, "_agn_model") and parameters._agn_model == "skirtor"
            ),
        )
    )
