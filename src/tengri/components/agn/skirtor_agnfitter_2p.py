...
"""SKIRTOR-averaged (mean_2p) clumpy torus library (Stalevski et al. 2016).

Two-parameter torus library interpolated differentiably in
``(agn_oa_skirtor, agn_incl_skirtor)`` over a precomputed HDF5 grid derived
from AGNfitter-rX's ``SKIRTOR_mean_2p.pickle``.

Sits between :mod:`tengri.components.agn.skirtor_agnfitter_1p`
(``SKIRTOR_mean_1p``, inclination only) and
:mod:`tengri.components.agn.skirtor_agnfitter` (``SKIRTOR_mean_3p``, the
AGNfitter-faithful default that adds the equatorial optical depth axis).
All three share the same opening-angle (10-80 deg) and inclination
(0-90 deg) extents.
...
__all__ = [
    "create_skirtor_agnfitter_2p_from_grid",
    "skirtor_agnfitter_2p_sed",
]


def _load_skirtor_agnfitter_2p_arrays(grid_path: str) -> dict:
    """Load raw numpy arrays from the ``SKIRTOR_mean_2p`` grid HDF5.

    Parameters
    ----------
    grid_path : str
        Path to ``skirtor_mean2p_torus_grid.h5``.

    Returns
    -------
    dict
        Keys ``oa_axis``, ``incl_axis``, ``wavelength``, and ``template``.
...
@functools.cache
def load_skirtor_agnfitter_2p_grid(grid_path: str) -> TorusTemplateGrid:
    """Load a ``SKIRTOR_mean_2p`` grid HDF5 into a :class:`TorusTemplateGrid` pytree.
    ...
    -------
    TorusTemplateGrid
        Template arrays with axes ``(oa, incl)``, as numpy arrays.
    ...
    **JIT-compatible**: no, performs HDF5 I/O. Call outside the trace and
    ...
    return TorusTemplateGrid(
        ...
        axes=(np.asarray(raw["oa_axis"]), np.asarray(raw["incl_axis"])),
        wave_grid=np.asarray(raw["wavelength"]),
    )


def skirtor_agnfitter_2p_sed_from_grid(
    ...
    **_kwargs,
    ...
    r"""``SKIRTOR_mean_2p`` torus SED at a single (oa, incl) node.
    ...
    .. math::
        ...
        L_\nu(\lambda) = L_{\rm bol}\,f_{\rm torus}\,
                         \frac{T(\lambda;\,\theta,\,i)}
                              {\int T(\nu;\,\theta,\,i)\,\mathrm{d}\nu}

    where :math:`\theta` is the half-opening angle and :math:`i` is
    inclination.

    **JIT-compatible**: yes.

    **Approximation**: the grid is drawn from the ``SKIRTOR_mean_2p``
    library (Stalevski et al. 2016 [1]_, [2]_) as packaged by
    AGNfitter-rX [3]_, which averages out clumpiness (p, q) and optical
    depth of the full SKIRTOR parameter space.
    """
...
def create_skirtor_agnfitter_2p_from_grid(grid_path: str) -> Callable:
    """Load a ``SKIRTOR_mean_2p`` grid and bind it to the SED evaluator.
    ...
    return functools.partial(
        skirtor_agnfitter_2p_sed_from_grid, load_skirtor_agnfitter_2p_grid(grid_path)
    )
...
_NOT_FOUND_MSG = (
    "SKIRTOR_mean_2p torus grid not found. Build it with: "
    "python scripts/build_agnfitter_torus_reductions.py --which skirtor_mean2p"
)


def _find_skirtor_agnfitter_2p_grid() -> str:
    from tengri._data_setup import require_data

    return require_data(_GRID_SEARCH_PATHS[-1], _NOT_FOUND_MSG)


def load_skirtor_agnfitter_2p_default_grid() -> TorusTemplateGrid:
    """Load the packaged ``SKIRTOR_mean_2p`` grid pytree (discovery + cache).
    ...
    ------
    FileNotFoundError
        If no ``SKIRTOR_mean_2p`` grid HDF5 is present on disk.
    """
    return load_skirtor_agnfitter_2p_grid(_find_skirtor_agnfitter_2p_grid())


@functools.cache
def _load_skirtor_agnfitter_2p_default() -> Callable:
    return create_skirtor_agnfitter_2p_from_grid(_find_skirtor_agnfitter_2p_grid())


def skirtor_agnfitter_2p_sed(
    *args, _template: TorusTemplateGrid | None = None, **kwargs
) -> jnp.ndarray:
    """``SKIRTOR_mean_2p`` torus (auto-loaded from the packaged HDF5 grid).
    ...
    **kwargs
    ...
    location, call :func:`create_skirtor_agnfitter_2p_from_grid` directly.
    """
    if _template is not None:
        return skirtor_agnfitter_2p_sed_from_grid(_template, *args, **kwargs)
    fn = _load_skirtor_agnfitter_2p_default()
