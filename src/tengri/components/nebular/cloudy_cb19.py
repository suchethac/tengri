    ...
    **JIT-compatible**: no: load-time only.
    ...
    if finite.size == 0 or float(finite.min()) == float(finite.max()):
        ...
        raise CB19DegenerateGridError(
            ...
            "(#2181). Fix (one of):\n"
            "  1. Supply your own grid: point at it with "
            "neb={'type': 'cb19', 'grid': <path>} (#2220), or place a CB_19 "
            "HDF5 file with real variation at data/cb19_templates.h5 (or "
            "under $TENGRI_DATA_DIR) so the default resolves it, matching "
    ...
    **JIT-compatible**: no, HDF5 I/O is not JAX-compatible. Call once
    ...
    if not filepath.exists():
        raise FileNotFoundError(
            ...
            "Supply your own grid: point at it with "
            "neb={'type': 'cb19', 'grid': <path>} (#2220), or place a CB_19 "
            "HDF5 file with real variation at this path (or under "
            "$TENGRI_DATA_DIR) so the default resolves it, matching the "
            "layout in docs/internal/advanced/cb19_grid.md. As of "
            "2026-09 the upstream 3MdB CB_19 table (ref='CB_19') is "
            "unpopulated pending the 3MdB team's own corrected grid and "
            "erratum, so there is currently no download route that can "
...
def check_cb19_free_params(
    ...
    **JIT-compatible**: no: composition-time only.
    ...
    raise ParameterError(
        ...
        "point at it with neb={'type': 'cb19', 'grid': <path>} (#2220), or "
        "place a CB_19 HDF5 file at data/cb19_templates.h5 (or under "
        "$TENGRI_DATA_DIR) so the default resolves it, matching the layout "
        "in docs/internal/advanced/cb19_grid.md. As of 2026-09 the upstream "
