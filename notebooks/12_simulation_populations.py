...
#
# ## What you will do
# Compute broadband photometry and emission-line fluxes for thousands of galaxies from a simulation. Each galaxy's star-formation and metallicity history are given as arrays (not fitted parameters), and you will generate observables fast enough for large populations.
#
# ## What you need
# An SSP grid with baked-in nebular emission (wNE), simulated or tabular star-formation and metallicity histories for N galaxies, and a filter set, without priors or fitting.
#
# ## What you will have
...
# ---
#
# A simulation provides star-formation and metallicity histories as arrays for each galaxy, not fitted parameters. The goal is broadband photometry and emission-line fluxes for all of them, as fast as possible. There is no fitting: the tables are the model. The working code occupies the first three sections; the final two sections benchmark speed across different approximation choices. Vectorization transforms the computation from one-galaxy-at-a-time to population-at-a-time, measured in seconds for millions of galaxies on a GPU.
...
# bytes are the currency that matters: the dependable win is the
...
# z = 3. At z = 6 the Lyman-break dropout bands are tiny (float64
...
# and `Catalog.from_histories` correctly refuses the whole catalog, since
