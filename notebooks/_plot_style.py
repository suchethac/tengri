...
def setup_style():
    ...
    Notebook defaults (tengri galleries): ``figure.dpi`` 200 for sharp previews,
    default ``figure.figsize`` (6, 4) inches, ``legend.fontsize`` 12. Saved
    figures still use ``savefig.dpi`` 300.
    ...
    plt.rcParams.update(
        {
            # Figure — crisp notebook / screen preview; export still uses savefig.dpi
            "figure.dpi": 200,
...
def plot_sfh(
    ...
    if posterior.samples is not None:
        ...
        if show_draws:
            ...
            for j in idx:
                ax.plot(t_gyr, sfh_arr[j], color=color, alpha=0.06, lw=0.5, rasterized=True)
    ...
    if posterior.samples is not None and model.spec.stochastic:
        ax_in = add_sfh_inset(ax, t_gyr, np.median(sfh_arr, axis=0), color=color, lw=1.2)
        if true_params is not None:
            ...
            if mask.sum() > 2:
                ax_in.plot(
                    t_myr[mask], np.asarray(sfh_t[t_key])[mask], color=COLORS["truth"], lw=1.5
                )
...
def plot_sfh_comparison(model, results, true_params=None, methods=None, figsize=(12.0, 3.8)):
...
def plot_sed_fit(
    ...
    if show_residuals:
        ...
        if ax is None:
            fig, ax_main = plt.subplots(figsize=(7.0, 3.8))
        else:
            ax_main = ax
            fig = ax.figure
...
def plot_corner_comparison(posteriors, labels, colors=None, truths=None, params=None):
    ...
    # Force all posteriors to use the same parameter list.
    # If not specified, use params from the first posterior that works.
    if params is None:
    ...
    # Add legend from diagonal labels
    if fig is not None and axes is not None:
        handles, leg_labels = [], []
        for ax in np.diag(axes):
            for h, l in zip(*ax.get_legend_handles_labels()):
                if l not in leg_labels:
                    handles.append(h)
                    leg_labels.append(l)
        if handles:
