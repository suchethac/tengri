...
#
# **Problem:** Dust attenuation curves describe how stellar light is dimmed at each wavelength,
# varying dramatically across galaxy types (starburst vs quiescent) and cosmic epochs. Prospector,
# BAGPIPES, and CIGALE each hardcode a fixed attenuation law; `tengri` lets you switch between
# 15 empirical and physics-motivated curves, explore the effect of key parameters (UV bump
# strength, slope, clumpiness), and compose them with a two-component model (birth-cloud + ISM)
# to match your SED. This notebook shows every curve side-by-side, builds intuition for when
# each is appropriate, and demonstrates the **age-dependent attenuation transition** that
# couples dust and star-formation-history degeneracies.
#
# **Prereqs:** 01_sed_anatomy | **Continue with:** 04_nebular_gallery, 07_fitting_photometry
#
# ---
#
# **Technical setup:** Each curve $k(\lambda)$ is normalized at 5500 A (V-band).
# The two-component model (Charlot & Fall 2000) applies separate curves to birth clouds
# (dust-enshrouded young stars, exponential age decay) and diffuse ISM:
#
# $$\tau(\lambda, \mathrm{age}) = w(\mathrm{age})\,\tau_\mathrm{bc}\,k_\mathrm{bc}(\lambda)
#   + \tau_\mathrm{diff}\,k_\mathrm{diff}(\lambda)$$
#
# where $w(\mathrm{age})$ is a sigmoid transition at $t_\mathrm{birth}$ (typically ~10 Myr).
#
# **Sections:**
#
# 1. Overview — all curves at default parameters
# 2. Empirical average curves (Calzetti, Leitherer)
# 3. Modified Calzetti family (Kriek & Conroy 2013, Noll et al. 2009, Salim et al. 2018)
# 4. MW/SMC/LMC extinction curves (reddening vs attenuation)
# 5. Physics-motivated curves (TEA, Conroy 2010, Narayanan 2018)
# 6. Parameter exploration (UV bump, slope, Li et al. 2008 $c_1$–$c_4$)
# 7. Dust geometries (Witt & Gordon 2000 clumpy models)
# 8. Two-component model (age-dependent transition, birth-cloud vs ISM)
# 9. Age–dust degeneracy (color-magnitude illustration)
# %%
...
ax.plot(
    np.array(wave_ext), np.array(k_lmc), color=COLORS["mcmc_nuts"], lw=2.5, label="LMC (Pei 1992)"
)
