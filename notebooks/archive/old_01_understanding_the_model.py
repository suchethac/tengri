...
# tengri imports
from tengri.components.sfh.psd_models import psd_drw, drw_acf, drw_variance, psd_to_sqrt_power
...
ax.set_ylabel(r"GP field (t)$ (offset)")
...
for name, p in archetypes.items():
    ...
    ax.plot(age_gyr, sfr, lw=2.2, color=p["color"],
            label=rf"{name} ($\alpha$={p['alpha']}, $\beta$={p['beta']}, "
                  rf"$\tau$={p['tau_gyr']} Gyr)")
...
for name, r in regimes.items():
    ...
    sfr_mean = double_powerlaw(ages_yr, alpha=1.5, beta=1.0, tau=5e9, norm=3.0)

    for draw in range(5):
        key, subkey = jax.random.split(key)
        xi = jax.random.normal(subkey, shape=(N_GRID,))
        gp = gp_from_xi(xi, sqrt_p, N_GRID)
        sfr_full = sfr_mean * jnp.exp(gp - k0_half)
        ax.plot(np.array(ages_gyr), np.array(sfr_full),
                lw=0.5, alpha=0.4, color=r["color"])

    ax.plot(np.array(ages_gyr), np.array(sfr_mean),
            "k--", lw=2, label="Mean SFH")
    ax.set_xscale("log")
    ax.set_xlim(1e-3, 14)
    ax.set_xlabel("Lookback time (Gyr)")
    ax.set_ylabel(r"SFR (M-r{\odot}$/yr)")
