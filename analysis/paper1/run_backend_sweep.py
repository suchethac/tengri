...
mcmc_hmc, and nss (nested slice sampling). Saves results to <out-dir>/<method>.npz + .json and a
...
given; the default is all five. ``--out-dir`` defaults to
...
#: The paper's backend list (owner decision 2026-08-30, updated 2026-09-14): MAP,
#: Laplace, NUTS (both automatic selector and explicit), fixed-length HMC, and
#: nested slice sampling. Ray Tracing has been removed from the paper.
SWEEP_METHODS = ("map", "laplace", "mcmc", "mcmc_nuts", "mcmc_hmc", "nss")
    ...
    for method in methods:
        ...
        try:
            ...
            if method == "map":
                ...
                # Nested slice sampling: evidence inference with posterior samples.
                posterior = forward.fit(data, key=key, method="nss")
                t_cold = time.perf_counter() - t_start
                t_warm = None
            ...
            # Record divergence count for samplers that report it (#2089).
            # Paper adoption bar: zero divergences AND max R-hat < 1.01.
            if posterior.diagnostics is not None:
                if "n_divergent" in posterior.diagnostics:
                    results_dict["n_divergent"] = int(posterior.diagnostics["n_divergent"])
                elif "n_divergences" in posterior.diagnostics:
                    results_dict["n_divergent"] = int(posterior.diagnostics["n_divergences"])
