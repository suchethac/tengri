...
def load_ssp_for(key: str) -> tengri.SSPData:
    """Load SSP grid for configuration key I/II/III/IV/V/VI."""
    ssp_names = {
        "I": "bc03_pdva_stelib_chabrier",
        "II": "ssp_prsc_miles_chabrier_wNE_logGasU-3.0_logGasZ0.0",
        "III": "fsps_mist_c3k_a_chabrier",
        "IV": "fsps_mist_c3k_a_chabrier",
        "V": "pgny_mist_c3k_chabrier",
        "VI": "bpss_stars_c3k_a_chabrier",
    }
...
def config_II(ssp_data: tengri.SSPData, observation, z: float) -> SEDModel:
    """Config II: DPL SFH, two-component Calzetti, DL07, SSP nebular (wNE grid baked in)."""
    return SEDModel.build(
        ssp_data=ssp_data,
        observation=observation,
        sfh={
...
def config_V(ssp_data: tengri.SSPData, observation, z: float) -> SEDModel:
    """Config V: Declining exponential SFH, Salim attenuation, Casey2012 dust, CUE nebular."""
    return SEDModel.build(
        ssp_data=ssp_data,
        observation=observation,
        sfh={
...
def config_VI(ssp_data: tengri.SSPData, observation, z: float) -> SEDModel:
    """Config VI: Dirichlet-weighted nonparametric SFH, Li08 attenuation, DL14 dust,
    CUE nebular.

    The Dirichlet family places its weights on age bins constructed from the cosmic
    age at the fitted redshift, so star formation cannot be placed before the Big
    Bang by construction. An exponential-rise parameterization was tried first and
    formed a large fraction of its mass outside cosmic time, which the forward model
    truncates, so the prediction did not reflect the requested history.

    This is a different SFH PRIOR from Configuration III's continuity model
    (Dirichlet weights on bin fractions, against a Student-t prior on log SFR
    ratios), which is the point: the choice of star formation prior is itself a
    modeling choice the catalog exposes.
    """
    bin_edges_gyr = tengri.make_agebins_from_zred(z, n_bins=7)
    sfh_dict = {
        "type": "dirichlet",
...
CONFIGS = {
    ...
    "IV": {
        "key": "IV",
        "name": "constant star formation",
        "ssp_grid": "fsps_mist_c3k_a_chabrier",
        "n_free": None,
    },
    "V": {
        "key": "V",
        "name": "declining exponential",
        "ssp_grid": "pgny_mist_c3k_chabrier",
        "n_free": None,
    },
    "VI": {
        "key": "VI",
...
if __name__ == "__main__":
    ...
    for cfg_key in ["I", "II", "III", "IV", "V", "VI"]:
