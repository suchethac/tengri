...
def plot_gradient_heatmap(jac, param_names, filter_names):
    ...
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 0.5 * n_params + 2),
                                     gridspec_kw={"width_ratios": [1, 1]})

    # Left: raw Jacobian (log scale)
    mat_log = np.sign(mat) * np.log10(np.abs(mat) + 1e-35)
    im1 = ax1.imshow(mat_log, aspect="auto", cmap="RdBu_r",
    ...
    label_map = {
        "sfh_alpha": r"$\alpha$ (falling slope)",
        "sfh_beta": r"$\beta$ (rising slope)",
        "sfh_tau_peak_gyr": r"$\tau_{\rm peak}$ (Gyr)",
        "sfh_peak_sfr": r"SFR$_{\rm peak}$",
        "psd_sigma": r"$\sigma_{\rm PSD}$",
        "psd_tau_myr": r"$\tau_{\rm PSD}$ (Myr)",
    ...
    im2 = ax2.imshow(np.abs(mat_norm), aspect="auto", cmap="YlOrRd",
                     vmin=0, vmax=1)
...
def main():
    ...
    param_names = [k for k in sorted(params.keys())
                   if k != "psd_xi" and k not in ("dust_slope", "redshift")]
