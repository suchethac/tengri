...
def generate_population(model_factory, psd_sigma, psd_tau_myr,
                        n_galaxies, key, snr=20.0):
...
def run_convergence_test(model_factory, psd_sigma_true, psd_tau_true,
                         n_values, method, key, **fit_kwargs):
    ...
    all_galaxies = generate_population(
        model_factory, psd_sigma_true, psd_tau_true,
        n_max, key, snr=20.0,
    )
    ...
    for n in n_values:
        ...
        hfitter = HierarchicalFitter(
            model_factory, galaxies_n,
            psd_sigma_prior=(0.1, 4.0),
            psd_tau_prior=(1.0, 300.0),
        )

        key_fit = jax.random.fold_in(key, n)
        result = hfitter.run(method, key=key_fit, verbose=False,
                             **fit_kwargs)

        s = result.summary()
        print(f"σ={s['psd_sigma']['median']:.2f} "
...
def run_distinction_test(model_factory, regimes, n_galaxies, method,
                         key, **fit_kwargs):
    ...
    for name, (sigma, tau) in regimes.items():
        ...
        galaxies = generate_population(
            model_factory, sigma, tau, n_galaxies, key, snr=20.0,
        )
...
def plot_results(convergence_results, distinction_results,
                 sigma_true, tau_true, distinction_truths):
    ...
    ax1.errorbar(n_values,
                 sigma_meds,
                 yerr=[np.array(sigma_meds) - np.array(sigma_los),
                       np.array(sigma_his) - np.array(sigma_meds)],
                 fmt="o-", color="C0", capsize=4, label=r"$\sigma_{\rm PSD}$")
    ax1.axhline(sigma_true, color="C0", ls="--", lw=1, alpha=0.5)

    # τ_PSD convergence (right y-axis)
    ax1b = ax1.twinx()
    ax1b.errorbar(np.array(n_values) + 1,  # slight offset for clarity
                  tau_meds,
                  yerr=[np.array(tau_meds) - np.array(tau_los),
                        np.array(tau_his) - np.array(tau_meds)],
                  fmt="s-", color="C1", capsize=4, label=r"$\tau_{\rm PSD}$ (Myr)")
    ...
    for name, result in distinction_results.items():
        ...
        ax2.scatter(sigma_samp, tau_samp, s=8, alpha=0.3,
        ...
        if len(sigma_samp) > 10:
            try:
                ...
                kde = gaussian_kde(xy)
                x_grid = np.linspace(sigma_samp.min() - 0.3,
                                     sigma_samp.max() + 0.3, 60)
                y_grid = np.linspace(tau_samp.min() - 20,
                                     tau_samp.max() + 20, 60)
                ...
                if len(levels) >= 2:
                    ax2.contour(X, Y, Z, levels=levels[:-1],
        ...
        ax2.plot(st, tt, "+", color=colors.get(name, "gray"),
                 ms=15, mew=2.5, zorder=10, label=f"{name} (truth)")
...
def main():
    ...
    parser.add_argument("--n-max", type=int, default=30,
                        help="Maximum N for convergence test")
    ...
    convergence_results = run_convergence_test(
        model_factory, sigma_true, tau_true, n_values,
        args.method, jax.random.fold_in(key, 100), **fit_kwargs,
    )
    ...
    distinction_results = run_distinction_test(
        model_factory, distinction_regimes, n_distinction,
        args.method, jax.random.fold_in(key, 200), **fit_kwargs,
    )
    ...
    fig = plot_results(
        convergence_results, distinction_results,
        sigma_true, tau_true, distinction_regimes,
    )
