...
from common import (
    make_model, generate_mock_galaxy, setup_matplotlib,
...
def benchmark_method(model, galaxy, method, key, **kwargs):
    ...
    if method == "map":
        ...
        # MAP init first
        result_map = fitter.run("map", key=key, n_steps=500,
                                learning_rate=0.03, verbose=False)
        key = jax.random.fold_in(key, 1)
        result = fitter.run(method, init_from=result_map, key=key,
...
def run_benchmarks(n_repeats: int = 3):
    ...
    for config_name, config_kwargs in configs.items():
        ...
        galaxy = generate_mock_galaxy(model, jax.random.fold_in(key, abs(hash(config_name)) % (2**31)),
                                      snr=20.0)
            ...
            for rep in range(n_repeats):
                ...
                try:
                    wt, diag = benchmark_method(
                        model, galaxy, method, rep_key, **kwargs,
                    )
                    times.append(wt)
                    last_diag = diag
                    print(f"{wt:.1f}s", end="")
...
def plot_speed_comparison(results):
    ...
    width = 0.18
    colors = {"MAP (Adam)": "#1f77b4", "Ray Tracing": "#ff7f0e",
              "NUTS": "#2ca02c", "geoVI": "#d62728"}
