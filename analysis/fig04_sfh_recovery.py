...
def run_single_recovery(
    psd_regime: str, method: str, data_type: str, key, n_grid: int = 128, **fit_kwargs
):
...
def plot_sfh_recovery_grid(results_phot, results_spec, n_mocks: int, method: str):
    ...
    fig, axes = plt.subplots(n_regimes, 2, figsize=(10, 3.0 * n_regimes), sharex=True)
    ...
    for row, regime in enumerate(regimes):
        for col, (data_type, results, color) in enumerate(
            ...
            for model, galaxy, fit_result in results:
                ...
                ax.fill_between(t_gyr, sfr_lo, sfr_hi, color=color, alpha=0.15)
    ...
    fig.suptitle(
        f"SFH Recovery ({method.title()}, {n_mocks} mock(s) per regime)", fontsize=14, y=1.01
    )
    plt.tight_layout()
    return fig
...
def compute_summary_table(results_phot, results_spec, models_phot, models_spec):
    ...
    for regime in PSD_REGIMES:
        for dtype, results, _models in [
            ...
            rmses_recent = []
            coverages = []
            for model, _galaxy, fit_result in results:
...
def main():
    ...
    parser.add_argument(
        "--n-mocks", type=int, default=3, help="Number of mocks per regime (default: 3)"
    )
    parser.add_argument(
        "--method",
        type=str,
    ...
    for regime in PSD_REGIMES:
        ...
        for i in range(args.n_mocks):
            ...
            print(
                f"RMSE={metrics_p['rmse_log_sfr']:.3f}, "
            ...
            print(
                f"RMSE={metrics_s['rmse_log_sfr']:.3f}, "
    ...
    fig = plot_sfh_recovery_grid(results_phot, results_spec, args.n_mocks, args.method)
    ...
    print(f"\n{'=' * 60}")
    print("Summary:")
    rows = compute_summary_table(results_phot, results_spec, models_phot, models_spec)
    print(
        f"{'Regime':<16s} {'Data':>5s} {'RMSE(log SFR)':>14s} {'RMSE(recent)':>13s} {'Cov68%':>7s}"
    )
    print("-" * 58)
    for r in rows:
        print(
            f"{r['regime']:<16s} {r['data_type']:>5s} "
