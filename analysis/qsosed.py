...
class Quasar_sed:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if LogL3000 is not None:
            self.f3000 = 10 ** (LogL3000 - four_pi_dL_sq(self.z)) / (3000 * (1 + self.z))
    ...
    def set_continuum(self, flxnrm=1.0, wavnrm=5500):
        ...
        fluxtemp = np.where(
            self.wavlen < wavbrk1, pl(self.wavlen, sl1, const1), pl(self.wavlen, sl2, const2)
        )
        ...
        self.flux = np.where(self.wavlen < wavbrk3, pl(self.wavlen, sl3, const3), fluxtemp)
    ...
    def add_emission_lines(self, wavnrm=5500, wmin=6000, wmax=7000):
        ...
        if scalin >= 0:
            ...
            self.flux += (
                _scatmp * linval * self.flux[self.wav2num(wavnrm)] / conval[self.wav2num(wavnrm)]
            )
