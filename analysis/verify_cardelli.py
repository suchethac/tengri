...
def cardelli_reference(wavelength_aa: np.ndarray, r_v: float = 3.1) -> np.ndarray:
    ...
    if np.any(uv):
        ...
        if np.any(fuv):
            y = xu[fuv] - 5.9
            a[uv] = np.where(
                xu >= 5.9, a[uv] + (-0.04473 * (xu - 5.9) ** 2 - 0.009779 * (xu - 5.9) ** 3), a[uv]
            )
            b[uv] = np.where(
                xu >= 5.9, b[uv] + (0.2130 * (xu - 5.9) ** 2 + 0.1207 * (xu - 5.9) ** 3), b[uv]
            )
...
from tengri.dust.attenuation import cardelli as tengri_cardelli
...
def main() -> None:
    ...
    print(f"\nV-band (5500 A): tengri={ds[v_idx]:.6f}  ref={ref[v_idx]:.6f}  (should be ~1.0)")
    ...
    if diff_valid.max() < 1e-10:
        ...
        print(
            f"     Worst at {ww:.0f} A (x={1e4 / ww:.2f}): "
    ...
    if np.any(fuv):
        ...
        print(
            f"  ref (clipped) range: [{ref_clipped[fuv].min():.4f}, {ref_clipped[fuv].max():.4f}]"
        )
        ...
        for ww in sample_waves:
            ...
            print(
                f"  {wave_aa[idx]:8.0f} {x[idx]:8.3f} {ds[idx]:10.4f} "
    ...
    if np.any(neg):
        ...
        print(f"  Wavelength range: {wave_aa[neg].min():.0f} - {wave_aa[neg].max():.0f} A")
        print(f"  Min reference value (before clip): {ref[neg].min():.4f}")
    else:
        print("\n--- Clipping analysis ---")
        print("  Reference never goes negative in 900-30000 A range.")
    ...
    if np.any(fuv):
        ...
        if fuv_diff > 0.01:
            ...
            print("  tengri applies the UV formula (valid for 3.3 < x < 8.0)")
            print("  beyond its domain. CCM89 Table 4 defines separate")
            print("  polynomials for 8 < x < 10:")
            print("    y = x - 8.0")
            print("    a(x) = -0.070*y^3 + 0.137*y^2 - 0.628*y - 1.073")
            print("    b(x) =  0.374*y^3 - 0.420*y^2 + 4.257*y + 13.670")
            print(f"  Max error: {fuv_diff:.4f} in A(lambda)/A(V)")

    # Issue 2: Clipping hiding problems
    ds_noclip = np.asarray(_cardelli_noclip(jnp.array(wave_aa), r_v))
    goes_neg = ds_noclip < 0
    if np.any(goes_neg):
        issues_found = True
        print("\n[ISSUE 2] CLIPPING HIDES UNPHYSICAL VALUES")
        print(f"  Without clip, tengri cardelli goes negative at {goes_neg.sum()} points")
        print(
            f"  Wavelength range: {wave_aa[goes_neg].min():.0f} - {wave_aa[goes_neg].max():.0f} A"
        )
        print(f"  Min value (no clip): {ds_noclip[goes_neg].min():.4f}")
        print("  This is because the UV formula extrapolated beyond x=8")
        print("  produces unphysical negative extinction.")
