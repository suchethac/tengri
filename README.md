...
- **Apple Silicon**: Apple's own `jax-metal` (0.1.1, 2024-10) is not viable against this JAX version. The supported path is the community `jax-mps` plugin (MLX-backed, float32 only; needs `MLX_DISABLE_COMPILE=1`) -- see `notebooks/apple_mps.py` for setup and `bench/scripts/benchmark_float32_mps_parity.py` for the float32 accuracy check. Set `JAX_PLATFORMS=cpu` for any fit you intend to trust without it.
