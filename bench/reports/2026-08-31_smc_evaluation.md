# Tempered SMC: 14-19x the gradients, and one seed nothing else has reached

**Date:** 2026-08-31
**Verdict:** **SHIP AS `tier="experimental"`. DO NOT PROMOTE. IT IS NOT THE SPEED
ANSWER.** At its cheapest arm that still samples, `mcmc_smc` costs **14-19x**
preconditioned fixed-`L` HMC's gradient evaluations per draw and **2.7-2.8x** its
wall clock, and it clears the notebooks' bar (max split R-hat < 1.01 **and** zero
...
inner moves of `L=20` reaches **1.0021** on the healthy control, at **80x** the
...
* **The fixed ladder is not the cheap one.** It does remove the `while_loop` —
  ...
  **compiles 48% slower** and runs 28% slower than the adaptive schedule. Compile
  is 3-5% of an SMC fit either way, so the axis that makes NUTS expensive simply
...
arm is 7-8x the gradients and 1.2-1.5x the wall clock and does not.
...
| fixture | seed | config | wall s (contended) | grads / draw | max split R-hat | div | div rate | min ESS | ancestor ESS | rungs | distinct draws |
|---|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| nb05 | 0 | `hmc+precond L=20` | **135** | **20** | 1.2257 | 0 | 0.00% | 1.6 | – | – | 0.965 |
| nb05 | 0 | `smc+precond cheap` | 364 | 360 | **1.0177** | 2703 | 7.33% | 2.5 | 252 of 512 | 18 | 0.770 |
| nb05 | 1 | `hmc+precond L=20` | **151** | **20** | 1.3189 | 0 | 0.00% | 1.8 | – | – | 0.956 |
| nb05 | 1 | `smc+precond cheap` | 420 | 380 | 1.2483 | 2811 | 7.42% | 2.4 | 245 of 512 | 19 | 0.548 |
| nb05 | 2 | `smc+precond cheap` | 319 | 300 | 1.1321 | 3969 | 13.37% | 3.4 | 242 of 512 | 15 | 0.479 |
| nb05 | 7 | `hmc+precond L=20` | **113** | **20** | **1.0325** | 2 | 0.17% | **39.2** | – | – | 0.802 |
| nb05 | 7 | `smc+precond cheap` | 309 | 320 | 1.1030 | 2117 | 6.46% | 12.4 | 250 of 512 | 16 | 0.577 |
| ctl-dpl | 7 | `hmc+precond L=20` | **97** | **20** | **1.0161** | 6 | 0.50% | 31.7 | – | – | 0.827 |
| ctl-dpl | 7 | `smc+precond cheap` | 261 | 280 | 1.0391 | 2103 | 7.61% | 51.6 | 224 of 512 | 14 | 0.592 |
...
**1. SMC is 2.7-2.8x the wall clock and 14-19x the gradients.** The wall-clock
ratio is stable across three fixtures taken under the same load — 364/135 = 2.7,
420/151 = 2.8, 261/97 = 2.7 — which is the only form a contended box can support.
The gradient ratio is exact and load-independent: 360/20 = **18x**,
380/20 = **19x**, 280/20 = **14x**. Wall clock *understates* the gap because
SMC's gradients ride a 512-wide `vmap` and HMC's ride a 2-wide one, so the vector
unit absorbs roughly a factor of six of the eighteen. That is SMC's real speed
argument and Finding 3 is where it is measured directly.
...
stopped paying, and charges 14-19x the gradients to do it.
...
gradients per effective sample: preconditioned HMC `20 x 1200 / 1.6` = **15 318**;
SMC `360 x 1024 / 252` = **731**. That inversion is entirely HMC's ESS collapsing
to 1.6 rather than SMC becoming cheap — which is the honest form of the seed-0
result, and the reason Finding 4 is not simply "SMC lost".
...
arm that reproduces the one result SMC has. The 1-move arm's ratios — 7-8x the
...
## Finding 2 — compile is 5-9 s and 3-5% of the fit, so the axis that decides NUTS does not decide this
...
**3. Compile is 3-5% of an SMC fit, so neither number decides anything.**
Lower-plus-compile is 9.7 s (adaptive) or 10.3 s (fixed) against a run of 179 s
or 230 s: a **cold/warm ratio of about 1.05**, against NUTS's measured 4.0
(189.4 / 46.8). **SMC's cost is sampling, essentially all of it.** The
control-flow shape that dominates NUTS's wall clock is, for this sampler, a
rounding error — which also means the fixed ladder buys nothing on the axis it
was proposed for.
...
| ctl-dpl seed 7 | grads / draw | wall s | max split R-hat | min ESS | **ancestor ESS** | div rate | distinct draws |
|---|---:|---:|---:|---:|---:|---:|---:|
| `hmc+precond L=20` | **20** | **97** | 1.0161 | 31.7 | – | 0.50% | 0.827 |
| `smc+precond cheap` (adaptive) | 280 | 261 | 1.0391 | 51.6 | 224 of 512 | 7.61% | 0.592 |
| `smc+precond fixed16` (5 moves, `L=20`) | 1600 | 1149 | **1.0021** | **330.1** | **1 of 512** | 4.69% | 0.944 |
...
**And the two ESS columns disagree by five orders of magnitude on the same row.**
Gradients per effective sample reads **4 963** against the autocorrelation ESS and
**819 200** against the ancestor ESS. Neither is the truth: the autocorrelation
number ignores that every particle descends from one ancestor, and the ancestor
number ignores 1 600 gradients of rejuvenation per particle since. **A row where
the two diverge like this is a row whose effective sample size is unknown**, and
that is the honest thing to write down rather than picking whichever column
flatters the sampler.
...
**Caveat 3 — the SMC arm is one configuration of many.** `n_particles=512`,
...
**Caveat 4 — single galaxy only.** `mcmc_smc` is not in `_MCMC_VMAPPABLE` and was
...
**Caveat 5 — SNR 20 under quadrature.** Repeated from the header. A throughput
...
**Caveat 6 — one fit per cell, one seed per mock.** R-hat, ESS and rung counts are
...
# 2. Finding 2 - the metric ablation, on the non-tsnorm control. The pair is the
...
# 3. Every row scored, with the divergence rate against the RIGHT denominator
#    and the ancestor ESS printed beside the autocorrelation one.
.venv/bin/python bench/scripts/score_smc_campaign.py \
    bench/results/2026-08-31_smc_campaign.jsonl

# 4. Compile anatomy: adaptive while_loop against fixed-ladder scan. The cache
...
# 5. Particle-width throughput. A FIXED ladder, so every width does equal work
...
# 6. Finding 5 - is the autocorrelation ESS measuring anything? A permutation
...
per `(notebook, config, seed)`), `2026-08-31_smc_compile_cpu.json`,
...
solve a problem the incumbent no longer has, and charges 14-19x the gradients.**
