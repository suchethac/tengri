...
def main():
    ...
    print()
    print(
        f"{'Parameter':<30} {'True':>8} {'Adam':>8} {'L-BFGS':>8} "
    ...
    for name in free_names:
        ...
        print(
            f"{name:<30} {true_val:>8.3f} {adam_val:>8.3f} {lbfgs_val:>8.3f} "
    ...
    print(
        f"{'Final loss':.<30} {adam_diag['final_loss']:>15.6f} {lbfgs_diag['final_loss']:>15.6f}"
    )
    print(f"{'Steps/iterations':.<30} {adam_diag['n_steps']:>15d} {lbfgs_diag['n_steps']:>15d}")

    if "converged" in lbfgs_diag:
        print(f"{'Converged':.<30} {'—':>15} {'yes' if lbfgs_diag['converged'] else 'no':>15}")
    if "grad_norm" in lbfgs_diag:
        print(f"{'Grad norm':.<30} {'—':>15} {lbfgs_diag['grad_norm']:>15.2e}")

    print(
        f"{'Mean |param error|':.<30} "
        f"{np.mean(adam_errors):>14.2f}% {np.mean(lbfgs_errors):>14.2f}%"
    )
    print(
        f"{'Max |param error|':.<30} {np.max(adam_errors):>14.2f}% {np.max(lbfgs_errors):>14.2f}%"
    )
    ...
    if lbfgs_loss < adam_loss:
        print(
            f"L-BFGS reaches {(adam_loss - lbfgs_loss) / adam_loss * 100:.1f}% lower loss "
            f"and is {adam_w / lbfgs_w:.1f}x {'faster' if lbfgs_w < adam_w else 'slower'} "
            f"(warm)."
        )
    else:
        print(
            f"Adam reaches {(lbfgs_loss - adam_loss) / lbfgs_loss * 100:.1f}% lower loss "
