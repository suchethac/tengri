...
- **`sfh={'age_kernel': ...}` is NOT a speed knob** (2026-07-31, #964). Selects how the SFH is integrated onto the SSP age grid: `'cic'` (default, dense cloud-in-cell integrand) or `'dsps'` (DSPS's histogram kernel). Menu: `tengri.list_age_kernels()`. DSPS's kernel is ~1.3x cheaper **in isolation**, but the age-weight kernel is <1% of a fit's gradient — measured `1.0026x` on a full `predict_photometry` gradient against a **1.76% noise floor**, i.e. not measurable. It is not worth switching for speed. It is also not equivalent: DSPS interpolates `log10(M(<t))` in `log10(t)`, which annihilates the first SSP node older than the SFH start (3.8% of the mass, +1.2% optical CSP bias vs FSPS/bagpipes) and shifts the `sfh_*_age_gyr` **gradient by 43%**. Use `'dsps'` only for cross-code parity. `'cic'` + a GP-field SFH raises (the field draw has no dense integrand).
