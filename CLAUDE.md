...
- **`FeaturePrecomp` has TWO consumers — ask which one you mean** (2026-08-13, #1748 + #1770). (1) The **nebular continuum shortcut** serves photometry from a per-Q_H grid, which requires zeroing `sed_nebular`; since #1281 that is permitted only when nothing downstream reads the continuum, and `DustSEDComponent` declares it as an input, so **any dusty model disarms this one**. Measured on a photometry gradient, dust-free control in the same run: with dust `WavePrecomp` 65,438,628 vs the pair 65,438,628 (**1.00x, bit-identical** — exact FLOP equality is the signature of a config that never reaches the graph); without dust 54,827,036 vs 1,789,868 (**30.6x**). (2) The **line-flux channel** is independent and **dust does not touch it**: `loss_functions` sets `needs_state = ... or (has_line_fluxes and not fast_lines)`, so the LUT is what stops every likelihood evaluation rebuilding the full-grid SED to read a few line fluxes (#1477). Measured on the **fit objective** of a dusty model: 2,173,351 → 465,023 (**4.67x**) — larger than the dust-free 3.89x, because the full-grid forward it avoids costs more when there is dust to apply. #1760 gated both consumers on the continuum predicate and silently withdrew the second; #1770 split them. Neither `WavePrecomp` (163.5M → 65.4M on that dusty model) nor the #1281 correctness fix is in question: restoring the old shortcut is not an option, since pre-#1281 it made a dusty model's photometry differ from exact by **0.41%** against a 0.0115% dust-free floor, and a constant forward bias enters the gradient multiplied by SNR (#1671). Predicates: `fast_nebular_can_engage` answers (1) only — built on the same expression that sets `must_materialize_sed`; `feature_precomp_can_engage` is the one a resolver should ask. **Measure the channel you are gating**: a `predict_photometry` probe cannot see the line channel, which is exactly how the over-broad gate shipped.
