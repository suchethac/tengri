...
- **`FeaturePrecomp` does nothing on a model with dust** (2026-08-13, #1748). It serves the nebular channel from a per-Q_H grid, which requires zeroing `sed_nebular` — and since #1281 that is permitted only when nothing downstream reads the continuum. `DustSEDComponent` declares it as an input, so **any dusty model disarms it**. Measured, gradient FLOPs off the compiled HLO with a dust-free control in the same run: with dust `WavePrecomp` 65,438,628 vs the pair 65,438,628 (**1.00x, bit-identical**); without dust 54,827,036 vs 1,789,868 (**30.6x**). Exact FLOP equality is the signature of a config that never reaches the graph. This is not a regression to restore: pre-#1281 the shortcut made a dusty model's photometry differ from exact by **0.41%** against a 0.0115% dust-free floor, and a constant forward bias enters the gradient multiplied by SNR (#1671). The fit resolvers now skip it where it cannot engage — otherwise the fit paid a second compiled kernel (`compile_signature()` differs) for no change in FLOPs. `WavePrecomp` itself is unaffected and still pays (163.5M → 65.4M on that dusty model). The predicate is `fast_nebular_can_engage`, built on the same expression that sets `must_materialize_sed`, so the advice cannot drift from the behavior again.
