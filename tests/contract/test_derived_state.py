...
"""Tests for :class:`tengri.protocols.DerivedState`.
...
These tests assume :class:`DerivedState` is a *drop-in* replacement
...
from tengri.protocols import DerivedState
...
class TestDictCompat:
    def test_getitem_present(self):
        b = DerivedState(L_ir=jnp.asarray(3.0))
    ...
    def test_get_with_default(self):
        ...
        b = DerivedState(L_ir=jnp.asarray(2.0))
    ...
    def test_keys_only_populated_fields(self):
        b = DerivedState(L_ir=jnp.asarray(1.0), sfr=jnp.asarray(2.0))
...
class TestExtrasSpillover:
    ...
    def test_unknown_key_via_from_dict_lands_in_extras_when_opted_in(self):
        b = DerivedState.from_dict({"L_ir": 1.0, "future_key": "anything"}, allow_extras=True)
        assert b["L_ir"] == 1.0
    ...
    def test_from_dict_strict_raises_on_unknown_key(self):
        ...
        with pytest.raises(TypeError, match="unknown key"):
            DerivedState.from_dict({"L_ir": 1.0, "future_key": "anything"})
...
class TestPytreeRegistration:
    def test_tree_flatten_unflatten_roundtrip(self):
        ...
        assert isinstance(b2, DerivedState)
...
class TestPackageRootExport:
    def test_at_tengri_core(self):
        import tengri.protocols as core

        assert core.DerivedState is DerivedState


class TestPhase2Flip:
    """ForwardState.derived is now DerivedState-typed (ADR-0007 Phase 2).
    ...
    def test_default_derived_is_a_bundle(self):
        ...
        assert isinstance(s.derived, DerivedState)
    ...
    def test_construct_with_dict_coerces_to_bundle(self):
        ...
        # get a DerivedState automatically via __post_init__.
        from tengri.protocols import ForwardState
    ...
    def test_with_dict_coerces_to_bundle(self):
        ...
        assert isinstance(s2.derived, DerivedState)
    ...
    def test_with_bundle_passes_through(self):
        # New-style write: pass a DerivedState directly. No coercion
        # needed, identity preserved by ``replace``.
        from tengri.protocols import ForwardState

        s = ForwardState(wave=jnp.linspace(1000.0, 10000.0, 8))
        b = DerivedState(L_ir=jnp.asarray(9.0))
    ...
    def test_unknown_key_in_dict_raises_after_phase4(self):
        ...
        # DerivedState.from_dict(..., allow_extras=False), so passing
