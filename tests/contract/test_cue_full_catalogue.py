...
"""Contract tests for the Cue ``full_catalogue`` opt-in (#303).
...
to the full ~271-species catalogue the Cue NN was trained on.
...
class TestGrammarPlumbing:
    """``neb={'type': 'cue', 'full_catalogue': True}`` reaches Parameters."""

    def test_default_is_false(self):
        params = parse_groups(neb={"type": "cue", "*": tengri.FIXED}, redshift=Fixed(0.05))
        assert params.cue_full_catalogue is False

    def test_opt_in_is_true(self):
        params = parse_groups(
            neb={"type": "cue", "*": tengri.FIXED, "full_catalogue": True},
            redshift=Fixed(0.05),
        )
        assert params.cue_full_catalogue is True
    ...
    def test_opt_in_only_affects_cue(self):
        ...
        params = parse_groups(
            neb={"type": "cb19", "*": tengri.FIXED, "full_catalogue": True},
            redshift=Fixed(0.05),
        )
        # Allowed by the validator, but not interpreted for cb19.
        assert params.cue_full_catalogue is False

    def test_full_catalogue_is_a_recognised_neb_key(self):
...
class TestConfigDefault:
    ...
    def test_default_false(self):
        ...
        assert cfg.cue_full_catalogue is False

    def test_constructor_opt_in(self):
        cfg = NebularSEDComponentConfig(backend="cue", cue_full_catalogue=True)
        assert cfg.cue_full_catalogue is True
    ...
    def test_field_is_frozen(self):
        ...
        with pytest.raises(Exception):  # noqa: B017 — dataclass FrozenInstanceError
            cfg.cue_full_catalogue = True  # type: ignore[misc]
