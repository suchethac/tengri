...
def _build(ssp, obs, **dust_extra):
    dust = {"type": "two_component", "*": tengri.FIXED,
            "tau_bc": 1.0, "tau_diff": 0.0, "emission": None}
...
class TestPerComponentSlopeWiring:
    def test_slope_bc_changes_prediction(self, synthetic_ssp_wide, synthetic_tophat_obs):
        ...
        steep = _phot(
            _build(synthetic_ssp_wide, synthetic_tophat_obs, slope_bc=-1.0)
        )
        assert not np.array_equal(steep, base), (
            "slope_bc=-1.0 produced no change in photometry — wiring is a no-op"
        )

    def test_default_equals_explicit_minus_07(
        ...
        explicit = _phot(
            _build(
                ...
                slope_bc=-0.7, slope_diff=-0.7,
            )
        )
        np.testing.assert_allclose(explicit, base, rtol=1e-6)

    def test_bluest_band_fainter_with_steeper_bc(
        self, synthetic_ssp_wide, synthetic_tophat_obs
    ):
...
class TestLawInheritance:
    def test_diff_only_inherits_to_bc(self, synthetic_ssp_wide, synthetic_tophat_obs):
        ...
        model = _build(
            synthetic_ssp_wide, synthetic_tophat_obs, law_diff="calzetti"
        )
    ...
    def test_bc_only_inherits_to_diff(self, synthetic_ssp_wide, synthetic_tophat_obs):
        ...
        model = _build(
            synthetic_ssp_wide, synthetic_tophat_obs, law_bc="calzetti"
        )
        assert model.spec.dust_law_bc == "calzetti"
        assert model.spec.dust_law_diff == "calzetti"

    def test_neither_defaults_power_law(
        self, synthetic_ssp_wide, synthetic_tophat_obs
    ):
