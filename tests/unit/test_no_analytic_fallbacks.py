...
# ---------------------------------------------------------------------------
# Analytic fallback functions must no longer be reachable via public API
# ---------------------------------------------------------------------------


class TestAnalyticFallbacksNotInvoked:
    """Confirm the analytic fallback functions still exist internally
    (for reference / documentation) but are NOT called by any public path
    when templates are absent — only FileNotFoundError is raised.
    """

    def test_dl07_analytic_fallback_not_called_on_missing_templates(self, monkeypatch):
        monkeypatch.setattr(_emission_mod, "_find_dl07_templates", lambda: None)
        _emission_mod._resolved.discard("draine_li2007")
        _emission_mod.DUST_EMISSION_MODELS["draine_li2007"] = _emission_mod._dl07_lazy_wrapper

        call_log = []
        original = _emission_mod._draine_li2007_analytic_fallback

        def spy(*a, **kw):
            call_log.append(True)
            return original(*a, **kw)

        monkeypatch.setattr(_emission_mod, "_draine_li2007_analytic_fallback", spy)
        ...
        assert len(call_log) == 0, "Analytic fallback must NOT be called — FileNotFoundError first"

    def test_dl14_analytic_fallback_not_called_on_missing_templates(self, monkeypatch):
        monkeypatch.setattr(_emission_mod, "_find_data_file", lambda *a, **k: None)
        _emission_mod._dl14_fn = None
        _emission_mod._resolved.discard("draine_li2014")
        _emission_mod.DUST_EMISSION_MODELS["draine_li2014"] = _emission_mod._dl14_lazy_wrapper
        ...
        call_log = []
        original = _emission_mod._draine_li2014_analytic_fallback

        def spy(*a, **kw):
            call_log.append(True)
            return original(*a, **kw)

        monkeypatch.setattr(_emission_mod, "_draine_li2014_analytic_fallback", spy)
