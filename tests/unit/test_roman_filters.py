...
class TestRomanFiltersRegistry:
    ...
    ROMAN_FILTERS = ["roman_f062", "roman_f087", "roman_f106", "roman_f129",
...
class TestRomanFilterData:
    ...
    def test_roman_filter_has_min_points(self, filter_name, expected_pivot_aa):
        ...
        curve = load_filter(filter_name)
        assert len(curve.wave) >= 50, \
            f"{filter_name} has only {len(curve.wave)} points"
    ...
    def test_roman_filter_transmission_bounds(self, filter_name, expected_pivot_aa):
        ...
        assert trans_np.min() >= -0.01, \
            f"{filter_name} transmission min {trans_np.min()} < 0"
        # Allow higher bounds for normalized SVO filters
        assert trans_np.max() <= 3.0, \
            f"{filter_name} transmission max {trans_np.max()} > 3.0"
    ...
    def test_roman_filter_peak_transmission(self, filter_name, expected_pivot_aa):
        ...
        peak = trans_np.max()
        assert peak > 0.3, \
            f"{filter_name} peak transmission {peak} < 0.3"
...
class TestRomanFilterSet:
    ...
    def test_load_roman_filter_set(self):
        ...
        filter_names = ["roman_f062", "roman_f087", "roman_f106", "roman_f129",
                        "roman_f158", "roman_f184", "roman_f213"]
