...
class TestDustLaws:
    ...
    def test_all_laws_return_positive(self, wave):
        ...
        for name, fn in DUST_LAWS.items():
            k = fn(wave, n_slope=-0.7, dust_bump_strength=1.0, dust_delta=0.0, dust_Rv=3.1)
    ...
    def test_f_obscuration_floor(self):
        ...
        atten = two_component_dust(wave, ages, tau_v1=2.0, tau_v2=1.0, f_obscuration=0.2)
...
class TestDustEmission:
    ...
    def test_energy_conservation(self, ir_wave):
        ...
        for name, fn in DUST_EMISSION_MODELS.items():
            ...
            ratio = L_total / L_abs
            assert 0.5 < ratio < 2.0, f"{name}: L_emitted/L_absorbed = {ratio:.2f}, expected ~1.0"
    ...
    def test_emission_gradient(self, ir_wave):
        ...
        g = jax.grad(lambda T: jnp.sum(modified_blackbody(ir_wave, 1e10, dust_T=T)))(35.0)
...
class TestDL14Emission:
    ...
    def test_dl14_alpha_default_matches_dl07(self, ir_wave):
        ...
        L_abs = 1e10
        sed_07 = draine_li2007(ir_wave, L_abs, dust_umin=1.0, dust_gamma_dl=0.01, dust_qpah=2.5)
    ...
    def test_dl14_jit_compatible(self, ir_wave):
        ...
        fn = jax.jit(
            lambda u, g, q, a: draine_li2014(
                ir_wave, 1e10, dust_umin=u, dust_gamma_dl=g, dust_qpah=q, dust_alpha_dl14=a
            )
        )
    ...
    def test_dl14_param_spec_registered(self):
        ...
        from tengri.core.param_spec import _DUST_EMISSION_PARAMS
...
class TestAGN:
    ...
    def test_agn_gradient(self, wave):
        ...
        g = jax.grad(lambda ll: jnp.sum(AGN_MODELS["simple"](wave, agn_log_lbol=ll)))(44.0)
        assert jnp.isfinite(g) and abs(float(g)) > 0
