...
class TestResolveShortNamesDPL:
    ...
    def test_log_peak_sfr_expands_for_dpl(self) -> None:
        result = resolve_short_names("dpl", {"log_peak_sfr": Uniform(-1.0, 3.0)})
        assert "sfh_dpl_log_peak_sfr" in result
        assert "log_peak_sfr" not in result
