# SPDX-License-Identifier: BSD-3-Clause
"""Tests for SEDModel.build resolver consulting _REGISTRY for SEDModelComponent ports."""

import pytest

from tengri import SEDModel, FREE, FIXED, Fixed, Uniform
from tengri.components.sed_model_component import _REGISTRY


class TestBuildResolverDustAttenuation:
    """Test that SEDModel.build resolves dust attenuation SEDModelComponent types."""

    def test_dust_calzetti_from_registry(self, ssp_data):
        """Build with dust type 'calzetti' from registry."""
        assert "calzetti" in _REGISTRY
        # Should not raise ValueError for unknown type
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"type": "calzetti", "tau_v": Fixed(0.3)},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_smc_from_registry(self, ssp_data):
        """Build with dust type 'smc' from registry."""
        assert "smc" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"type": "smc", "tau_v": Fixed(0.2)},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_mw_from_registry(self, ssp_data):
        """Build with dust type 'mw' (Milky Way) from registry."""
        assert "mw" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"type": "mw", "tau_v": Fixed(0.3)},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_salim18_from_registry(self, ssp_data):
        """Build with dust type 'salim18' from registry."""
        assert "salim18" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"type": "salim18", "tau_v": Fixed(0.3)},
            redshift=Fixed(0.1),
        )
        assert model is not None


class TestBuildResolverDustEmission:
    """Test that SEDModel.build resolves dust emission SEDModelComponent types."""

    def test_dust_emission_modified_blackbody(self, ssp_data):
        """Build with dust emission type 'modified_blackbody_ir' from registry."""
        assert "modified_blackbody_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "modified_blackbody_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_emission_dl07(self, ssp_data):
        """Build with dust emission type 'dl07_ir' from registry."""
        assert "dl07_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "dl07_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_emission_dl14(self, ssp_data):
        """Build with dust emission type 'dl14_ir' from registry."""
        assert "dl14_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "dl14_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_emission_dale2014(self, ssp_data):
        """Build with dust emission type 'dale2014_ir' from registry."""
        assert "dale2014_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "dale2014_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_emission_astrodust(self, ssp_data):
        """Build with dust emission type 'astrodust_ir' from registry."""
        assert "astrodust_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "astrodust_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_dust_emission_draine2021_pah(self, ssp_data):
        """Build with dust emission type 'draine2021_pah_ir' from registry."""
        assert "draine2021_pah_ir" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            dust={"emission": {"type": "draine2021_pah_ir"}},
            redshift=Fixed(0.1),
        )
        assert model is not None


class TestBuildResolverNebular:
    """Test that SEDModel.build resolves nebular SEDModelComponent types."""

    def test_neb_cue_emulator(self, ssp_data):
        """Build with nebular type 'cue_emulator' from registry."""
        assert "cue_emulator" in _REGISTRY
        # Should not raise ValueError for unknown type
        model = SEDModel.build(
            ssp_data=ssp_data,
            neb={"type": "cue_emulator"},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_neb_cloudy_grid(self, ssp_data):
        """Build with nebular type 'cloudy_grid' from registry."""
        assert "cloudy_grid" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            neb={"type": "cloudy_grid"},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_neb_cb19(self, ssp_data):
        """Build with nebular type 'cb19' from registry."""
        assert "cb19" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            neb={"type": "cb19"},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_neb_mappings(self, ssp_data):
        """Build with nebular type 'mappings' from registry."""
        assert "mappings" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            neb={"type": "mappings"},
            redshift=Fixed(0.1),
        )
        assert model is not None


class TestBuildResolverAGNTorus:
    """Test that SEDModel.build resolves AGN torus SEDModelComponent types."""

    def test_agn_torus_skirtor(self, ssp_data):
        """Build with AGN torus type 'skirtor' from registry."""
        assert "skirtor" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            agn={"torus": {"type": "skirtor"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_agn_torus_silva04(self, ssp_data):
        """Build with AGN torus type 'silva04' from registry."""
        assert "silva04" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            agn={"torus": {"type": "silva04"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_agn_torus_cat3d_wind(self, ssp_data):
        """Build with AGN torus type 'cat3d_wind' from registry."""
        assert "cat3d_wind" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            agn={"torus": {"type": "cat3d_wind"}},
            redshift=Fixed(0.1),
        )
        assert model is not None


class TestBuildResolverAGNDisc:
    """Test that SEDModel.build resolves AGN disc SEDModelComponent types."""

    def test_agn_disc_kd18(self, ssp_data):
        """Build with AGN disc type 'kd18_disc' from registry."""
        assert "kd18_disc" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            agn={"disc": {"type": "kd18_disc"}},
            redshift=Fixed(0.1),
        )
        assert model is not None

    def test_agn_disc_powerlaw(self, ssp_data):
        """Build with AGN disc type 'powerlaw_disc' from registry."""
        assert "powerlaw_disc" in _REGISTRY
        model = SEDModel.build(
            ssp_data=ssp_data,
            agn={"disc": {"type": "powerlaw_disc"}},
            redshift=Fixed(0.1),
        )
        assert model is not None
