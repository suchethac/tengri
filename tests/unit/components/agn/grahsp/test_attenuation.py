...
def test_smc_curve_matches_upstream(fixture):
    ...
    out = np.asarray(
        smc_attenuation_curve(wave_nm, opt_index=-1.2, nir_index=-3.0,
                              norm=1.2, lam_break_nm=1100.0)
    )
...
def test_fawcett_curve_matches_upstream(fixture):
    ...
    out = np.asarray(
        smc_attenuation_curve(wave_nm, opt_index=-1.0, nir_index=-2.6,
                              norm=1.0, lam_break_nm=1100.0)
    )
    np.testing.assert_allclose(out, expected, rtol=1e-12)
...
def test_curve_continuity_at_break():
    ...
    out = smc_attenuation_curve(
        np.array([1100.0]), opt_index=-1.2, nir_index=-3.0,
        norm=1.2, lam_break_nm=1100.0,
    )
...
def test_attenuation_factors(fixture):
    ...
    for i, case in enumerate(cases):
        f_gal, f_agn = attenuation_factors(
            wave_nm, ebv=float(case["ebv"]),
            ebv_agn=float(case["ebv_agn"]),
            opt_index=-1.2, nir_index=-3.0,
            norm=1.2, lam_break_nm=1100.0,
        )
        np.testing.assert_allclose(np.asarray(f_gal), expected_gal[i], rtol=1e-12,
                                    err_msg=f"gal case {i}")
        np.testing.assert_allclose(np.asarray(f_agn), expected_agn[i], rtol=1e-12,
                                    err_msg=f"agn case {i}")
...
def test_jit_compatible():
    ...
    f_gal, f_agn = fn(jnp.array([100.0, 1100.0, 10000.0]),
                       0.5, 0.3, -1.2, -3.0, 1.2, 1100.0)
