...
from tengri.components.agn.nlr import nlr_emission_richardson2014

jax.config.update("jax_enable_x64", True)


class TestRichardsonNLR:
    """Test suite for nlr_emission_richardson2014 function."""

    def test_output_shape(self):
        """Output shape matches input wavelength array."""
        wave = jnp.linspace(3000, 10000, 200)
        sed = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44)
    ...
    def test_zero_covering_fraction(self):
        ...
        wave = jnp.linspace(3000, 10000, 200)
        sed = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, covering_fraction=0.0)
        assert jnp.all(sed == 0.0)

    def test_jit_compatible(self):
        """Function is JIT-compilable."""
        wave = jnp.linspace(3000, 10000, 200)
        fn = jax.jit(nlr_emission_richardson2014)
    ...
    def test_scales_with_luminosity(self):
        ...
        wave = jnp.linspace(3000, 10000, 200)
        sed1 = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44)
        sed2 = nlr_emission_richardson2014(wave, l_disc_bol_erg=2e44)
    ...
    def test_scales_with_covering_fraction(self):
        ...
        wave = jnp.linspace(3000, 10000, 200)
        sed1 = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, covering_fraction=0.1)
        sed2 = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, covering_fraction=0.2)
        assert jnp.allclose(sed2, 2.0 * sed1, rtol=1e-5)
    ...
    def test_different_fwhm(self):
        ...
        sed_narrow = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, fwhm_kms=300)
        sed_broad = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, fwhm_kms=1000)
    ...
    def test_line_efficiency(self):
        ...
        wave = jnp.linspace(3000, 10000, 200)
        sed1 = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, line_efficiency=0.05)
        sed2 = nlr_emission_richardson2014(wave, l_disc_bol_erg=1e44, line_efficiency=0.10)
        assert jnp.allclose(sed2, 2.0 * sed1, rtol=1e-5)
    ...
    def test_vmap_compatibility(self):
        ...
        def compute_one_sed(l_bol):
            return nlr_emission_richardson2014(waves, l_disc_bol_erg=l_bol)
