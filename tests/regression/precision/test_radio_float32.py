...
_TRUTH = {"sfh_delayed_log_total_mass": 10.0}


def _model(ssp, radio):
    """A stellar + dust(+IR) + power-law-disc AGN + radio model. ``radio`` is the group.

    ``agn_log_lbol`` is pinned Fixed at 11.0 and that is the value the AGN is
    built at: this build used to carry ``fracAGN: 0.1`` as well, which under
    ...
    radio arm). The AGN jet arm's radio-loudness reference is derived from
    the pinned 11.0, so the ``fracAGN`` coupling was overwriting the one
    luminosity this file measures at. ``_TRUTH`` no longer names
    ``agn_log_lbol`` explicitly (#2296: a params-dict key the spec declared
    Fixed is refused, not merged) -- every prediction below omits it and gets
    the same pinned 11.0 from the spec automatically.
    """
