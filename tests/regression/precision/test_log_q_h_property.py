...
Validates that pred.log_q_h is finite in both float64 and float32. The linear
``q_h`` was retired with no alias (#1206 §C): it overflowed float32 at every
physical ionizing rate (~1e56 photons/s, past the 3.4e38 ceiling), so
``log_q_h`` is now the sole form -- accessing ``q_h`` raises, naming the
replacement.
...
def test_log_q_h_property(ssp_bare):
    ...
    assert "q_h" not in model.available_properties, (
        "'q_h' was retired with no alias (#1206 §C); it should not be registered"
    )
    ...
    # 3. Check finiteness (log_q_h) and that q_h raises, naming log_q_h
    ...
    with pytest.raises(KeyError, match="log_q_h"):
        _ = pred.q_h
    with pytest.raises(KeyError, match="log_q_h"):
        model.predict_properties(params, names=("q_h",))
    ...
    # 4. Pure float32: log_q_h finite (the whole point)
    # disable_jit keeps the forward eager so the float32 behavior is observable
