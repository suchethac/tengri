...
def test_the_two_surfaces_cannot_disagree_about_feature_lut_eligibility():
    ...
    **What this does NOT catch, measured rather than assumed.** Against
    ``74c31c23e`` — the last commit before #1683 was fixed — this test is
    GREEN, while the two per-surface value tests above fail. Both resolvers
    were broken *identically* there, so they agreed. An agreement test is blind
    to a shared error by construction; it catches one surface drifting from the
    other, which is the state #1596 -> #1683 passed through, not the state they
    both started in. That is exactly why the value assertions stay: neither
    test subsumes the other, and reading this one as "the #1683 guard" would
    leave the real cliff unpinned.
    """
    verdicts = {
        name: (
