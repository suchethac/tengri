...
# ===================================================================
# 8. BOSA — sSFR-temperature connection
# ===================================================================


class TestBOSAsSFRPhysics:
    """Higher sSFR must produce warmer dust (Boquien & Salim 2021)."""

    def test_higher_ssfr_warmer_peak(self):
        """Higher sSFR → peak at shorter wavelength (warmer dust)."""
        from tengri.models.dust.emission import _bosa_analytic_fallback

        wave = jnp.logspace(4, 7, 1000)  # 1-1000 um in A

        sed_low = _bosa_analytic_fallback(wave, 1e10, dust_log_ssfr=-11.0)
        sed_high = _bosa_analytic_fallback(wave, 1e10, dust_log_ssfr=-9.0)

        peak_low = float(wave[jnp.argmax(sed_low * wave)])
        peak_high = float(wave[jnp.argmax(sed_high * wave)])

        # Higher sSFR → shorter peak wavelength (warmer)
        assert peak_high < peak_low, (
            f"Higher sSFR should peak at shorter λ: "
            f"sSFR=-11→{peak_low / 1e4:.0f}um, sSFR=-9→{peak_high / 1e4:.0f}um"
        )

    def test_energy_conservation(self):
        """Total emitted IR must equal L_absorbed (in Lsun).

        The analytic fallback returns L_nu such that integral over
        frequency equals L_absorbed (in Lsun, same units as input).
        """
        from tengri.models.dust.emission import _bosa_analytic_fallback
        ...
        sed = _bosa_analytic_fallback(wave, l_absorbed, dust_log_ssfr=-10.0)

        # Integrate L_nu over frequency (result in same units as L_absorbed)
        c_aa = 2.99792458e18
        nu = c_aa / wave
        ...
        np.testing.assert_allclose(
            ...
            l_absorbed,
            rtol=0.15,
            err_msg=f"BOSA energy balance: emitted={l_bol:.3e}, absorbed={l_absorbed:.3e}",
        )


# ===================================================================
# 9. ASTRODUST — qPAH parameter physics
# ===================================================================


class TestAstrodustPhysics:
    """Astrodust emission energy conservation and basic physics."""

    def test_energy_conservation(self):
        """Astrodust total emission must equal L_absorbed."""
        from tengri.models.dust.emission import _astrodust_analytic_fallback

        wave = jnp.logspace(3.5, 7.5, 2000)
        l_absorbed = 1e10

        sed = _astrodust_analytic_fallback(wave, l_absorbed, dust_qpah=3.0)
        c_aa = 2.99792458e18
        nu = c_aa / wave
        l_bol = float(-jnp.trapezoid(sed, nu))
        ...
        np.testing.assert_allclose(
            ...
            l_absorbed,
            rtol=0.15,
            err_msg=f"Astrodust energy: emitted={l_bol:.3e}, absorbed={l_absorbed:.3e}",
        )

    def test_scales_linearly_with_luminosity(self):
        """Doubling L_absorbed should double the emission."""
        from tengri.models.dust.emission import _astrodust_analytic_fallback

        wave = jnp.logspace(4, 7, 500)

        sed1 = _astrodust_analytic_fallback(wave, 1e10, dust_qpah=3.0)
        sed2 = _astrodust_analytic_fallback(wave, 2e10, dust_qpah=3.0)

        ratio = sed2 / jnp.maximum(sed1, 1e-50)
        nonzero = sed1 > 1e-30
        np.testing.assert_allclose(
            np.asarray(ratio[nonzero]),
            2.0,
            rtol=0.01,
            err_msg="Emission should scale linearly with L_absorbed",
        )

    def test_higher_umin_warmer(self):
        """Higher U_min should shift SED peak to shorter wavelengths."""
        from tengri.models.dust.emission import _astrodust_analytic_fallback

        wave = jnp.logspace(4, 7, 1000)

        sed_low = _astrodust_analytic_fallback(wave, 1e10, dust_umin=0.5)
        sed_high = _astrodust_analytic_fallback(wave, 1e10, dust_umin=10.0)

        peak_low = float(wave[jnp.argmax(sed_low * wave)])
        peak_high = float(wave[jnp.argmax(sed_high * wave)])

        assert peak_high < peak_low, (
            f"Higher U_min should peak at shorter λ: "
            f"U=0.5→{peak_low / 1e4:.0f}um, U=10→{peak_high / 1e4:.0f}um"
        )


# ===================================================================
# 10. THEMIS — a-C(:H) grain properties
# ===================================================================


class TestTHEMISPhysics:
    """THEMIS dust emission basic physics and energy conservation."""

    def test_energy_conservation(self):
        """THEMIS total emission must equal L_absorbed."""
        from tengri.models.dust.emission import _themis_analytic_fallback

        wave = jnp.logspace(3.5, 7.5, 2000)
        l_absorbed = 1e10  # Lsun

        sed = _themis_analytic_fallback(wave, l_absorbed, dust_qhac=0.17)
        ...
        np.testing.assert_allclose(
            ...
            l_absorbed,
            rtol=0.15,
            err_msg=f"THEMIS energy: emitted={l_bol:.3e}, absorbed={l_absorbed:.3e}",
        )
    ...
    def test_scales_linearly_with_luminosity(self):
        ...
        sed1 = _themis_analytic_fallback(wave, 1e10, dust_qhac=0.17)
        sed2 = _themis_analytic_fallback(wave, 2e10, dust_qhac=0.17)
        ...
        np.testing.assert_allclose(
            ...
            2.0,
            rtol=0.01,
            err_msg="THEMIS emission should scale linearly with L_absorbed",
        )

    def test_non_negative_everywhere(self):
        """THEMIS SED must be non-negative at all wavelengths."""
        from tengri.models.dust.emission import _themis_analytic_fallback

        wave = jnp.logspace(3, 8, 2000)
        sed = _themis_analytic_fallback(wave, 1e10, dust_qhac=0.17)
        assert jnp.all(sed >= 0), "THEMIS SED has negative values"


# ===================================================================
# 11. Li08 attenuation — Li et al. (2008) Eq. (1), coefficients c1–c4
# ===================================================================
