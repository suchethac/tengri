...
class TestLineFluxBlockMeasurement:
    ...
    def test_fixture_grid_actually_has_nebular_emission(self, ssp_data_wne):
        ...
        """
        # Access the SSP grid arrays from the SSPData object.
        # Attributes: ssp_wave, ssp_flux, ssp_lg_age_gyr, ssp_lgmet, etc.
        wave = np.asarray(ssp_data_wne.ssp_wave)
        ssp_flux = np.asarray(ssp_data_wne.ssp_flux)  # shape: (n_age, n_met, n_wave)
        lgmet = np.asarray(ssp_data_wne.ssp_lgmet)  # log10(Z) absolute
        age_gyr = np.asarray(ssp_data_wne.ssp_lg_age_gyr)

        # Find the youngest age (highest log-age index) and nearest solar
        # metallicity (index where lgmet ~= log10(0.0142) ~= -1.848)
        youngest_age_idx = -1  # Last index = youngest
        solar_met_idx = np.argmin(np.abs(lgmet - (-1.848)))  # Closest to solar

        # Extract the SED at youngest age, solar metallicity
        sed = ssp_flux[youngest_age_idx, solar_met_idx, :]

        # Find Halpha flux: peak within +-8 Angstrom of 6562.8 A
        halpha_center = 6562.8
        halpha_mask = (wave >= halpha_center - 8.0) & (wave <= halpha_center + 8.0)
        halpha_flux = np.max(sed[halpha_mask])

        # Continuum level as mean over two windows away from lines
        cont_blue = np.mean(sed[(wave >= 6400) & (wave <= 6500)])
        cont_red = np.mean(sed[(wave >= 6650) & (wave <= 6750)])
        continuum_flux = (cont_blue + cont_red) / 2.0

        ratio = halpha_flux / continuum_flux

        # On wNE grids, Halpha/continuum is ~2-3. On bare grids, ~0.95 or less.
        assert ratio > 2.0, (
            f"Halpha/continuum ratio {ratio:.2f} suggests the grid is not wNE "
            f"(bare grid would be ~0.95). Check ssp_data_wne fixture."
        )
